;(function (root, factory) {
	...
	var semanticVersionIsOk = function (minimum_version, current_version) {
		...
		minimum_version_array.forEach(function (d, i) {
			if (
				!version_is_ok &&
				+current_version_array[i] > +minimum_version_array[i]
			) {
				version_is_ok = true
			}
		})
	...
	try {
		...
		LocusZoom.populate = function (selector, datasource, layout) {
			...
			d3.select(selector).call(function () {
				...
				if (
					typeof this.node().dataset !== 'undefined' &&
					typeof this.node().dataset.region !== 'undefined'
				) {
					var parsed_state = LocusZoom.parsePositionQuery(
						this.node().dataset.region
					)
		...
		LocusZoom.positionIntToString = function (pos, exp, suffix) {
			...
			var places_exp =
				exp - Math.floor((Math.log(pos) / Math.LN10).toFixed(exp + 3))
		...
		LocusZoom.prettyTicks = function (
			range,
			clip_range,
			target_tick_count
		) {
			if (
				typeof target_tick_count == 'undefined' ||
				isNaN(parseInt(target_tick_count))
			) {
				target_tick_count = 5
			}
			...
			var ticks = []
			var i = parseFloat(
				(Math.floor(range[0] / unit) * unit).toFixed(base_toFixed)
			)
			while (i < range[1]) {
				ticks.push(i)
				i += unit
			...
			if (
				typeof clip_range == 'undefined' ||
				['low', 'high', 'both', 'neither'].indexOf(clip_range) == -1
			) {
				clip_range = 'neither'
			}
		...
		LocusZoom.createCORSPromise = function (
			method,
			url,
			body,
			headers,
		...
		LocusZoom.validateState = function (new_state, layout) {
			...
			if (
				typeof new_state.chr != 'undefined' &&
				typeof new_state.start != 'undefined' &&
				typeof new_state.end != 'undefined'
			) {
				...
				if (isNaN(new_state.start) && isNaN(new_state.end)) {
					...
					attempted_scale = 0
					new_state.start = isNaN(new_state.start)
						? new_state.end
						: new_state.start
					new_state.end = isNaN(new_state.end)
						? new_state.start
						: new_state.end
				} else {
					attempted_midpoint = Math.round(
						(new_state.start + new_state.end) / 2
					)
			...
			if (
				!isNaN(layout.min_region_scale) &&
				validated_region &&
				attempted_scale < layout.min_region_scale
			) {
				new_state.start = Math.max(
					attempted_midpoint -
						Math.floor(layout.min_region_scale / 2),
					1
				)
			...
			if (
				!isNaN(layout.max_region_scale) &&
				validated_region &&
				attempted_scale > layout.max_region_scale
			) {
				new_state.start = Math.max(
					attempted_midpoint -
						Math.floor(layout.max_region_scale / 2),
					1
				)
		...
		LocusZoom.parseFields = function (data, html) {
			...
			var resolve = function (variable) {
				if (!resolve.cache.hasOwnProperty(variable)) {
					resolve.cache[variable] = new LocusZoom.Data.Field(
						variable
					).resolve(data)
				}
			...
			var render_node = function (node) {
				if (node.text) {
					...
					try {
						...
						console.error(
							'Error while processign condition ' +
								JSON.stringify(node.variable)
						)
					}
					return ''
		...
		LocusZoom.getToolTipData = function (node) {
			...
			if (
				selector.classed('lz-data_layer-tooltip') &&
				typeof selector.data()[0] != 'undefined'
			) {
			...
			var curtain = {
				...
				show: function (content, css) {
					...
					if (!this.curtain.showing) {
						this.curtain.selector = d3
							.select(this.parent_plot.svg.node().parentNode)
							.insert('div')
			...
			var loader = {
              ...
              */
				...
				update: function (content, percent) {
					...
					var page_origin = this.getPageOrigin()
					var loader_boundrect = this.loader.selector
						.node()
		...
		LocusZoom.Layouts = (function () {
			...
			obj.get = function (type, name, modifications) {
				if (typeof type != 'string' || typeof name != 'string') {
					...
					// Get the base layout
					var layout = LocusZoom.Layouts.merge(
						modifications || {},
						layouts[type][name]
					)
					...
					if (typeof layout.namespace == 'string') {
						...
						typeof layout.namespace == 'object' &&
						Object.keys(layout.namespace).length
					) {
						if (typeof layout.namespace.default != 'undefined') {
							...
							default_namespace =
								layout.namespace[
									Object.keys(layout.namespace)[0]
								].toString()
						}
					}
					...
					var applyNamespaces = function (element, namespace) {
						...
						if (typeof element == 'string') {
							...
							if (typeof element.namespace != 'undefined') {
								var merge_namespace =
									typeof element.namespace == 'string'
										...
										: element.namespace
								namespace = LocusZoom.Layouts.merge(
									namespace,
									merge_namespace
								)
							}
							...
							for (var property in element) {
								...
								namespaced_element = applyNamespaces(
									element[property],
									namespace
								)
								namespaced_property = applyNamespaces(
									property,
									namespace
								)
			...
			obj.set = function (type, name, layout) {
				...
				if (layout) {
					return (layouts[type][name] = JSON.parse(
			...
			obj.merge = function (custom_layout, default_layout) {
				if (
					typeof custom_layout != 'object' ||
					typeof default_layout != 'object'
				) {
					throw (
						'LocusZoom.Layouts.merge only accepts two layout objects; ' +
						typeof custom_layout +
						', ' +
				...
				for (var property in default_layout) {
					...
					var custom_type =
						custom_layout[property] == null
							? 'undefined'
							: typeof custom_layout[property]
					var default_type = typeof default_layout[property]
					if (
						custom_type == 'object' &&
						Array.isArray(custom_layout[property])
					) {
						custom_type = 'array'
					}
					if (
						default_type == 'object' &&
						Array.isArray(default_layout[property])
					) {
						default_type = 'array'
					}
					...
					if (custom_type == 'undefined') {
						custom_layout[property] = JSON.parse(
							JSON.stringify(default_layout[property])
						)
						continue
					}
					// Both values are objects: merge recursively
					if (custom_type == 'object' && default_type == 'object') {
						custom_layout[property] = LocusZoom.Layouts.merge(
							custom_layout[property],
							default_layout[property]
						)
						continue
					}
				}
...
*/
		...
		LocusZoom.Layouts.add('tooltip', 'standard_association', {
			...
			html:
				'<strong>{{{{namespace[assoc]}}variant}}</strong><br>' +
		...
		var covariates_model_association = LocusZoom.Layouts.get(
			'tooltip',
			'standard_association',
			{ unnamespaced: true }
		)
		covariates_model_association.html +=
...
*/
		...
		var covariates_model_plot_dashboard = LocusZoom.Layouts.get(
			'dashboard',
			'standard_plot'
		)
		...
		LocusZoom.Layouts.add(
			...
			covariates_model_plot_dashboard
		)

		var region_nav_plot_dashboard = LocusZoom.Layouts.get(
			'dashboard',
			'standard_plot'
		)
...
*/
		...
		LocusZoom.StandardLayout = LocusZoom.Layouts.get(
			'plot',
			'standard_association'
		)
...
*/
		...
		LocusZoom.DataLayer = function (layout, parent) {
			...
			if (
				...
				parent instanceof LocusZoom.Panel
			) {
				this.parent_plot = parent.parent
			}

			this.layout = LocusZoom.Layouts.merge(
				layout || {},
			...
			if (
				this.layout.x_axis != {} &&
				typeof this.layout.x_axis.axis != 'number'
			) {
				this.layout.x_axis.axis = 1
			}
			if (
				this.layout.y_axis != {} &&
				typeof this.layout.y_axis.axis != 'number'
			) {
			...
			if (this.parent) {
				...
				LocusZoom.DataLayer.Statuses.adjectives.forEach(
					function (status) {
		...
		LocusZoom.DataLayer.prototype.getElementId = function (element) {
			...
			return (this.getBaseId() + '-' + element_id).replace(
				/(:|\.|\[|\]|,)/g,
				'_'
			)
		}
		...
		LocusZoom.DataLayer.prototype.moveUp = function () {
			if (
				this.parent.data_layer_ids_by_z_index[this.layout.z_index + 1]
			) {
		...
		LocusZoom.DataLayer.prototype.resolveScalableParameter = function (
			layout,
			data
		) {
			var ret = null
					...
					case 'object':
						if (layout.scale_function) {
							if (layout.field) {
								...
								ret = LocusZoom.ScaleFunctions.get(
									layout.scale_function,
									layout.parameters || {},
									f.resolve(data)
								)
							} else {
		...
		LocusZoom.DataLayer.prototype.getAxisExtent = function (dimension) {
			...
			if (
				!isNaN(this.layout[axis].floor) &&
				!isNaN(this.layout[axis].ceiling)
			) {
			...
			if (this.layout[axis].field && this.data && this.data.length) {
				var extent = d3.extent(
					this.data,
					function (d) {
						var f = new LocusZoom.Data.Field(
							this.layout[axis].field
						)
						return +f.resolve(d)
					}.bind(this)
				)
				...
				if (
					isNaN(this.layout[axis].floor) &&
					!isNaN(this.layout[axis].lower_buffer)
				) {
					extent[0] -=
						original_extent_span * this.layout[axis].lower_buffer
				}
				if (
					isNaN(this.layout[axis].ceiling) &&
					!isNaN(this.layout[axis].upper_buffer)
				) {
					extent[1] +=
						original_extent_span * this.layout[axis].upper_buffer
				}
				...
				if (typeof this.layout[axis].min_extent == 'object') {
					if (
						isNaN(this.layout[axis].floor) &&
						!isNaN(this.layout[axis].min_extent[0])
					) {
						extent[0] = Math.min(
							extent[0],
							this.layout[axis].min_extent[0]
						)
					}
					if (
						isNaN(this.layout[axis].ceiling) &&
						!isNaN(this.layout[axis].min_extent[1])
					) {
						extent[1] = Math.max(
							extent[1],
							this.layout[axis].min_extent[1]
						)
					}
				}
		...
		LocusZoom.DataLayer.prototype.createTooltip = function (d, id) {
			if (typeof this.layout.tooltip != 'object') {
				throw (
					'DataLayer [' +
					this.id +
					'] layout does not define a tooltip'
				)
			}
		...
		LocusZoom.DataLayer.prototype.updateTooltip = function (d, id) {
			...
			if (this.layout.tooltip.html) {
				this.tooltips[id].selector.html(
					LocusZoom.parseFields(d, this.layout.tooltip.html)
				)
			}
		...
		LocusZoom.DataLayer.prototype.positionTooltip = function (id) {
			...
			if (!this.tooltips[id].arrow) {
				this.tooltips[id].arrow = this.tooltips[id].selector
					.append('div')
		...
		LocusZoom.DataLayer.prototype.showOrHideTooltip = function (element) {
			...
			var resolveStatus = function (statuses, directive, operator) {
				...
				if (Array.isArray(directive)) {
					...
					if (directive.length == 1) {
						...
						status = directive.reduce(function (
							previousValue,
							currentValue
						) {
							if (operator == 'and') {
								return (
									statuses[previousValue] &&
					...
					for (var sub_operator in directive) {
						sub_status = resolveStatus(
							statuses,
							directive[sub_operator],
							sub_operator
						)
						if (status == null) {
							status = sub_status
			...
			LocusZoom.DataLayer.Statuses.adjectives.forEach(
				function (status) {
					...
					statuses[status] =
						this.state[this.state_id][status].indexOf(id) != -1
		...
		LocusZoom.DataLayer.prototype.filter = function (filters, return_type) {
			if (
				typeof return_type == 'undefined' ||
				['indexes', 'elements'].indexOf(return_type) == -1
			) {
				return_type = 'indexes'
			}
		...
		LocusZoom.DataLayer.Statuses.verbs.forEach(function (verb, idx) {
			...
			LocusZoom.DataLayer.prototype[verb + 'Element'] = function (
				element,
				exclusive
			) {
			...
			LocusZoom.DataLayer.prototype[antiverb + 'Element'] = function (
				element,
				exclusive
			) {
				if (typeof exclusive == 'undefined') {
			...
			LocusZoom.DataLayer.prototype[verb + 'ElementsByFilters'] =
				function (filters, exclusive) {
					if (typeof exclusive == 'undefined') {
						exclusive = false
					} else {
						exclusive = !!exclusive
					}
					return this.setElementStatusByFilters(
						adjective,
						true,
						filters,
						exclusive
			...
			LocusZoom.DataLayer.prototype[antiverb + 'ElementsByFilters'] =
				function (filters, exclusive) {
					if (typeof exclusive == 'undefined') {
						exclusive = false
					} else {
			...
			LocusZoom.DataLayer.prototype[antiverb + 'AllElements'] =
				function () {
		...
		LocusZoom.DataLayer.prototype.setElementStatus = function (
			status,
			element,
			toggle,
			exclusive
			...
			if (
				...
				LocusZoom.DataLayer.Statuses.adjectives.indexOf(status) == -1
			) {
			...
			d3.select('#' + element_id).classed(
				'lz-data_layer-' + this.layout.type + '-' + status,
				toggle
			)
			if (this.layout.hover_element) {
				var hover_element_class =
					'lz-data_layer-' +
					this.layout.type +
					'-' +
					this.layout.hover_element +
					'-' +
					status
				var selector = d3.select(
				...
				if (this.layout.group_hover_elements_on_field) {
					selector =
						this.group_hover_elements[
							element[this.layout.group_hover_elements_on_field]
						]
				}
			...
			var element_status_idx =
				this.state[this.state_id][status].indexOf(element_id)
		...
		LocusZoom.DataLayer.prototype.setElementStatusByFilters = function (
			status,
			toggle,
			filters,
			exclusive
			...
			// Apply statuses
			this.filterElements(filters).forEach(
				function (element) {
		...
		LocusZoom.DataLayer.prototype.setAllElementStatus = function (
			status,
			toggle
		) {
			// Sanity check
		...
		LocusZoom.DataLayer.prototype.applyBehaviors = function (selection) {
			...
			Object.keys(this.layout.behaviors).forEach(
				function (directive) {
					var event_match = /(click|mouseover|mouseout)/.exec(
						directive
					)
					if (!event_match) {
						return
					...
					selection.on(
						event_match[0] + '.' + directive,
						this.executeBehaviors(
							directive,
							this.layout.behaviors[directive]
						)
					)
				}.bind(this)
			)
		...
		LocusZoom.DataLayer.prototype.executeBehaviors = function (
			directive,
			behaviors
		) {
			...
			return function (element) {
				...
				if (
					requiredKeyStates.ctrl != !!d3.event.ctrlKey ||
					requiredKeyStates.shift != !!d3.event.shiftKey
				) {
					return
				}
				...
				behaviors.forEach(
					function (behavior) {
						...
						switch (behavior.action) {
							...
							// Toggle a status
							case 'toggle':
								var current_status_boolean =
									this.state[this.state_id][
										behavior.status
									].indexOf(this.getElementId(element)) != -1
								var exclusive =
									behavior.exclusive &&
									!current_status_boolean
								this.setElementStatus(
									behavior.status,
									element,
									!current_status_boolean,
		...
		LocusZoom.DataLayer.prototype.exportData = function (format) {
			...
			format =
				typeof format == 'string'
					? format.toLowerCase()
					: default_format
			if (['json', 'csv', 'tsv'].indexOf(format) == -1) {
		...
		LocusZoom.DataLayer.prototype.reMap = function () {
			...
			// Fetch new data
			var promise = this.parent_plot.lzd.getData(
				this.state,
				this.layout.fields
			) //,"ld:best"
			promise.then(
			...
			obj.get = function (name, layout, parent) {
				if (!name) {
					...
					if (typeof layout != 'object') {
						throw (
							'invalid layout argument for data layer [' +
							name +
							']'
						)
					} else {
...
*/
		...
		LocusZoom.DataLayers.add('scatter', function (layout) {
			...
			this.positionTooltip = function (id) {
				...
				var point_size = this.resolveScalableParameter(
				...
				var page_origin = this.getPageOrigin()
				var x_center = this.parent.x_scale(
					tooltip.data[this.layout.x_axis.field]
				)
				var y_scale = 'y' + this.layout.y_axis.axis + '_scale'
				var y_center = this.parent[y_scale](
					tooltip.data[this.layout.y_axis.field]
				)
				var tooltip_box = tooltip.selector
					.node()
					.getBoundingClientRect()
				var data_layer_height =
					this.parent.layout.height -
					(this.parent.layout.margin.top +
						this.parent.layout.margin.bottom)
				var data_layer_width =
				...
				if (this.layout.tooltip_positioning == 'vertical') {
					...
					var offset_right = Math.max(
						tooltip_box.width / 2 - x_center,
						0
					)
					var offset_left = Math.max(
						tooltip_box.width / 2 + x_center - data_layer_width,
						0
					)
					var left =
						page_origin.x +
					...
					if (
						tooltip_box.height + stroke_width + arrow_width >
						data_layer_height - (y_center + offset)
					) {
						top =
							page_origin.y +
							y_center -
					...
					if (y_center - tooltip_box.height / 2 <= 0) {
						...
						top =
							...
							tooltip_box.height
						arrow_top =
							tooltip_box.height - 2 * arrow_width - border_radius
					} else {
						// vertically centered
				...
				// Create / update position on arrow connecting tooltip to data
				if (!tooltip.arrow) {
					tooltip.arrow = tooltip.selector
						.append('div')
						.style('position', 'absolute')
				}
				tooltip.arrow
			...
			this.flip_labels = function () {
				var data_layer = this
				var point_size = data_layer.resolveScalableParameter(
					data_layer.layout.point_size,
					{}
				)
				...
				var min_x = 2 * spacing
				var max_x =
					data_layer.parent.layout.width -
					data_layer.parent.layout.margin.left -
					data_layer.parent.layout.margin.right -
					2 * spacing
				...
				data_layer.label_texts.each(function (d, i) {
					...
					if (dax + abound.width + spacing > max_x) {
						var dal = handle_lines
							? d3.select(data_layer.label_lines[0][i])
							: null
						flip(da, dal)
					}
				})
				...
				data_layer.label_texts.each(function (d, i) {
					...
					data_layer.label_texts.each(function () {
						...
						var collision =
							abound.left <
								bbound.left + bbound.width + 2 * spacing &&
							abound.left + abound.width + 2 * spacing >
								bbound.left &&
							abound.top <
								bbound.top + bbound.height + 2 * spacing &&
							abound.height + abound.top + 2 * spacing >
								bbound.top
						if (collision) {
							flip(da, dal)
			...
			this.separate_labels = function () {
				...
				data_layer.label_texts.each(function () {
					...
					data_layer.label_texts.each(function () {
						...
						var min_y = 2 * spacing
						var max_y =
							data_layer.parent.layout.height -
							data_layer.parent.layout.margin.top -
							data_layer.parent.layout.margin.bottom -
							2 * spacing
						var delta
			...
			this.render = function () {
				...
				if (this.layout.label) {
					...
					var filtered_data = this.data.filter(function (d) {
						if (!data_layer.layout.label.filters) {
							...
							var match = true
							data_layer.layout.label.filters.forEach(function (
								filter
							) {
								var field_value = new LocusZoom.Data.Field(
									filter.field
								).resolve(d)
					...
					this.label_texts
						.text(function (d) {
							return LocusZoom.parseFields(
								d,
								data_layer.layout.label.text || ''
							)
						})
						...
						.attr({
							x: function (d) {
								var x =
									data_layer.parent[x_scale](
										d[data_layer.layout.x_axis.field]
									) +
									Math.sqrt(
										data_layer.resolveScalableParameter(
											data_layer.layout.point_size,
											d
										)
									) +
							...
							y: function (d) {
								var y = data_layer.parent[y_scale](
									d[data_layer.layout.y_axis.field]
								)
								if (isNaN(y)) {
									y = -1000
								}
					...
					if (data_layer.layout.label.lines) {
						...
						this.label_lines
							...
							.attr({
								x1: function (d) {
									var x = data_layer.parent[x_scale](
										d[data_layer.layout.x_axis.field]
									)
									if (isNaN(x)) {
										x = -1000
				...
				var initial_y = isNaN(this.parent.layout.height)
					? 0
					: this.parent.layout.height
				selection
					.enter()
			...
			this.makeLDReference = function (element) {
				...
				if (typeof element == 'undefined') {
					...
					if (
						...
						typeof element[this.layout.id_field] != 'undefined'
					) {
...
*/
		...
		LocusZoom.DataLayers.add('line', function (layout) {
			...
			this.getMouseDisplayAndData = function () {
				...
				ret.data[y_field] = interpolate(
					(ret.data[x_field] % range) / range
				)
				ret.display.y = this.parent[y_scale](ret.data[y_field])
				if (this.layout.tooltip.x_precision) {
					ret.data[x_field] = ret.data[x_field].toPrecision(
				...
				ret.slope =
					(this.parent[y_scale](endDatum[y_field]) -
						this.parent[y_scale](startDatum[y_field])) /
					(this.parent[x_scale](endDatum[x_field]) -
						this.parent[x_scale](startDatum[x_field]))
				return ret
			}
			...
			this.positionTooltip = function (id) {
				...
				var border_radius = 6 // as defined in the default stylesheet
				var stroke_width =
					parseFloat(this.layout.style['stroke-width']) || 1
				var page_origin = this.getPageOrigin()
				var data_layer_height =
					this.parent.layout.height -
					(this.parent.layout.margin.top +
				...
				if (Math.abs(dd.slope) > 1) {
					...
					if (dd.display.y - tooltip_box.height / 2 <= 0) {
						...
						dd.display.y + tooltip_box.height / 2 >=
						data_layer_height
					) {
						// Too close to the bottom, pull it up
						top =
					...
					var offset_right = Math.max(
						tooltip_box.width / 2 - dd.display.x,
						0
					)
					var offset_left = Math.max(
						tooltip_box.width / 2 + dd.display.x - data_layer_width,
						0
					)
					left =
						page_origin.x +
					...
					arrow_left =
						...
						offset_right
					arrow_left = Math.min(
						Math.max(arrow_left, min_arrow_left),
						max_arrow_left
					)
				...
				if (this.layout.tooltip) {
					...
					hitarea
						...
						.on('mousemove', function () {
							...
							data_layer.positionTooltip(
								data_layer.getElementId()
							)
						})
						.on('mouseout', function () {
							data_layer.tooltip_timeout = setTimeout(
								function () {
									data_layer.mouse_event = null
									data_layer.destroyTooltip(
										data_layer.getElementId()
									)
								},
								300
			...
			this.setAllElementStatus = function (status, toggle) {
				...
				if (
					typeof status == 'undefined' ||
					LocusZoom.DataLayer.Statuses.adjectives.indexOf(status) ==
						-1
				) {
					throw 'Invalid status passed to DataLayer.setAllElementStatus()'
				}
		...
		LocusZoom.DataLayers.add('genes', function (layout) {
					...
					function (d, g) {
						...
						this.data[g].display_range = {
							start: this.parent.x_scale(
								Math.max(d.start, this.state.start)
							),
							end: this.parent.x_scale(
								Math.min(d.end, this.state.end)
							),
						}
						this.data[g].display_range.label_width =
							this.getLabelWidth(
								this.data[g].gene_name,
								this.layout.label_font_size
							)
						this.data[g].display_range.width =
							this.data[g].display_range.end -
							this.data[g].display_range.start
							...
							if (d.start < this.state.start) {
								...
								if (
									...
									this.data[g].display_range.end =
										this.data[g].display_range.start +
										this.data[g].display_range.label_width
									this.data[g].display_range.text_anchor =
										'start'
								} else if (
									this.data[g].display_range.end +
										centered_margin >
						...
						this.data[g].display_domain.width =
							this.data[g].display_domain.end -
							this.data[g].display_domain.start
						...
						while (this.data[g].track == null) {
							...
							this.gene_track_index[potential_track].map(
								function (placed_gene) {
									if (!collision_on_potential_track) {
										var min_start = Math.min(
											placed_gene.display_range.start,
											this.display_range.start
										)
										var max_end = Math.max(
											placed_gene.display_range.end,
											this.display_range.end
										)
										if (
											max_end - min_start <
											placed_gene.display_range.width +
												this.display_range.width
										) {
							...
							if (!collision_on_potential_track) {
								...
								this.gene_track_index[potential_track].push(
									this.data[g]
								)
							} else {
								potential_track++
						...
						this.data[g].transcripts.map(
							function (d, t) {
								this.data[g].transcripts[t].parent =
									this.data[g]
								this.data[g].transcripts[t].exons.map(
									function (d, e) {
										this.data[g].transcripts[t].exons[
											e
										].parent = this.data[g].transcripts[t]
									}.bind(this)
								)
							}.bind(this)
						)
				...
				selection
					...
					.each(function (gene) {
						...
						var bboxes = d3
							.select(this)
							.selectAll(
								'rect.lz-data_layer-genes.lz-data_layer-genes-bounding_box'
							)
							.data([gene], function (d) {
								return d.gene_name + '_bbox'
							})
						...
						bboxes
							...
							.attr(
								'class',
								'lz-data_layer-genes lz-data_layer-genes-bounding_box'
							)

						bboxes
							.attr('id', function (d) {
								return (
									data_layer.getElementId(d) + '_bounding_box'
								)
							})
							.attr('rx', function () {
						...
						height = function () {
							return (
								data_layer.getTrackHeight() -
								data_layer.layout.track_vertical_spacing
							)
						}
						x = function (d) {
						...
						if (data_layer.canTransition()) {
							bboxes
								.transition()
								.duration(
									data_layer.layout.transition.duration || 0
								)
								.ease(
									data_layer.layout.transition.ease ||
										'cubic-in-out'
								)
								.attr('width', width)
								.attr('height', height)
						...
						width = function (d) {
							return (
								data_layer.parent.x_scale(d.end) -
								data_layer.parent.x_scale(d.start)
							)
						}
						height = function () {
						...
						y = function (d) {
							return (
								(d.track - 1) * data_layer.getTrackHeight() +
						...
						labels
							...
							.style(
								'font-size',
								gene.parent.layout.label_font_size
							)

						x = function (d) {
							if (d.display_range.text_anchor == 'middle') {
								return (
									d.display_range.start +
									d.display_range.width / 2
								)
						...
						var exons = d3
							...
							.data(
								gene.transcripts[gene.parent.transcript_idx]
									.exons,
								function (d) {
									return d.exon_id
								}
						...
						y = function () {
							return (
								(gene.track - 1) * data_layer.getTrackHeight() +
								data_layer.layout.bounding_box_padding +
								data_layer.layout.label_font_size +
								data_layer.layout.label_exon_spacing
							)
						...
						clickareas.on(
							'click.event_emitter',
							function (element) {
								element.parent.parent.emit(
									'element_clicked',
									element
								)
								element.parent.parent_plot.emit(
									'element_clicked',
									element
								)
							}
			...
			this.positionTooltip = function (id) {
				...
				var tooltip_box = tooltip.selector
					...
					.getBoundingClientRect()
				var gene_bbox_id =
					this.getElementId(tooltip.data) + '_bounding_box'
				var gene_bbox = d3
					.select('#' + gene_bbox_id)
					.node()
					.getBBox()
				...
				// pad to either side if bumping up against the edge of the data layer
				var gene_center_x =
					(tooltip.data.display_range.start +
						tooltip.data.display_range.end) /
						2 -
					this.layout.bounding_box_padding / 2
				var offset_right = Math.max(
					tooltip_box.width / 2 - gene_center_x,
					0
				)
				var offset_left = Math.max(
					tooltip_box.width / 2 + gene_center_x - data_layer_width,
					0
				)
				var left =
					page_origin.x +
				...
				if (
					tooltip_box.height + stroke_width + arrow_width >
					data_layer_height - (gene_bbox.y + gene_bbox.height)
				) {
					top =
						page_origin.y +
						gene_bbox.y -
		...
		LocusZoom.DataLayers.add('intervals', function (layout) {
			...
			this.assignTracks = function () {
				...
				if (this.layout.track_split_field && this.layout.split_tracks) {
					this.data.map(
						function (d) {
							this.track_split_field_index[
								d[this.layout.track_split_field]
							] = null
						}.bind(this)
					)
				...
				this.data.map(
					function (d, i) {
						...
						this.data[i].display_range = {
							start: this.parent.x_scale(
								Math.max(
									d[this.layout.start_field],
									this.state.start
								)
							),
							end: this.parent.x_scale(
								Math.min(
									d[this.layout.end_field],
									this.state.end
								)
							),
						}
						...
						if (
							...
							var val =
								this.data[i][this.layout.track_split_field]
							this.data[i].track =
								this.track_split_field_index[val]
							this.interval_track_index[this.data[i].track].push(
								i
							)
						} else {
							...
							while (this.data[i].track == null) {
								var collision_on_potential_track = false
								this.interval_track_index[potential_track].map(
									function (placed_interval) {
										if (!collision_on_potential_track) {
											var min_start = Math.min(
												placed_interval.display_range
													.start,
												this.display_range.start
											)
											var max_end = Math.max(
			...
			this.render = function () {
				...
				if (this.layout.split_tracks) {
					Object.keys(this.group_hover_elements).forEach(
						function (key) {
							if (!this.track_split_field_index[key]) {
					...
					Object.keys(this.track_split_field_index).forEach(
						function (key) {
							if (!this.group_hover_elements[key]) {
								this.group_hover_elements[key] = this.svg.group
									.insert('rect', ':first-child')
									.attr(
										'class',
										'lz-data_layer-intervals lz-data_layer-intervals-bounding_box'
									)
							}
							this.group_hover_elements[key]
								...
								.attr(
									'width',
									this.parent.layout.cliparea.width
								)
								.attr(
									'height',
									this.getTrackHeight() -
								...
								.attr(
									'y',
									(this.track_split_field_index[key] - 1) *
										this.getTrackHeight()
								)
						}.bind(this)
					)
				...
				selection
					...
					.each(function (interval) {
						...
						var bboxes = d3
							.select(this)
							.selectAll(
								'rect.lz-data_layer-intervals.lz-data_layer-intervals-bounding_box'
							)
							.data([interval], function (d) {
								return d[data_layer.layout.id_field] + '_bbox'
							})
						if (data_layer.layout.split_tracks) {
							...
							width = function (d) {
								return (
									d.display_range.width +
									2 * data_layer.layout.bounding_box_padding
								)
							}
							height = function () {
								return (
							...
							if (data_layer.canTransition()) {
								bboxes
									.transition()
									.duration(
										data_layer.layout.transition.duration ||
											0
									)
									.ease(
										data_layer.layout.transition.ease ||
						...
						fill = function (d) {
							return data_layer.resolveScalableParameter(
								data_layer.layout.color,
								d
							)
						}
						fill_opacity = function (d) {
							return data_layer.resolveScalableParameter(
								data_layer.layout.fill_opacity,
								d
							)
						}
			...
			this.positionTooltip = function (id) {
				...
				if (this.layout.split_tracks) {
					interval_bbox = d3
						.select('#' + this.getElementId(tooltip.data))
						.node()
						.getBBox()
				} else {
					interval_bbox = d3
						.select(
							'#' +
								this.getElementId(tooltip.data) +
								'_bounding_box'
						)
						.node()
						.getBBox()
				...
				// pad to either side if bumping up against the edge of the data layer
				var interval_center_x =
					(tooltip.data.display_range.start +
						tooltip.data.display_range.end) /
						2 -
					this.layout.bounding_box_padding / 2
				var offset_right = Math.max(
					tooltip_box.width / 2 - interval_center_x,
					0
				)
				var offset_left = Math.max(
					tooltip_box.width / 2 +
				...
				if (
					tooltip_box.height + stroke_width + arrow_width >
					data_layer_height - (interval_bbox.y + interval_bbox.height)
				) {
					top =
						page_origin.y +
						interval_bbox.y -
			...
			this.updateSplitTrackAxis = function () {
				var legend_axis = this.layout.track_split_legend_to_y_axis
				...
				if (this.layout.split_tracks) {
					...
					var target_height =
						tracks * track_height + (tracks - 1) * track_spacing
					...
					if (legend_axis && this.parent.legend) {
						...
						this.layout.legend.forEach(
							function (element) {
								...
								if (track) {
									if (
										this.layout.track_split_order == 'DESC'
									) {
										track = Math.abs(track - tracks - 1)
									}
									this.parent.layout.axes[
										legend_axis
									].ticks.push({
										y: track,
										text: element.label,
			...
			this.toggleSplitTracks = function () {
				...
				this.layout.group_hover_elements_on_field = this.layout
					.split_tracks
					? this.layout.track_split_field
					: null
			...
			this.render = function () {
				...
				chromosomes
					.attr('fill', function (d) {
						return d['genome:chr'] % 2
							? data_layer.layout.chromosome_fill_colors.light
							: data_layer.layout.chromosome_fill_colors.dark
					})
					.attr('x', function (d) {
			...
			this.render = function () {
				...
				if (
					...
					var ci_transform = function (d) {
						var x = this.parent[x_scale](
							d[this.layout.confidence_intervals.start_field]
						)
						var y = this.parent[y_scale](
							d[this.layout.y_axis.field]
						)
						if (isNaN(x)) {
							x = -1000
						}
					...
					var ci_width = function (d) {
						return (
							this.parent[x_scale](
								d[this.layout.confidence_intervals.end_field]
							) -
							this.parent[x_scale](
								d[this.layout.confidence_intervals.start_field]
							)
		...
		LocusZoom.KnownDataSources = (function () {
			...
			obj.add = function (source) {
				if (!source.SOURCE_NAME) {
					console.warn(
						'Data source added does not have a SOURCE_NAME'
					)
				}
				sources.push(source)
			}
			...
			obj.push = function (source) {
				console.warn(
					'Warning: KnownDataSources.push() is depricated. Use .add() instead'
				)
				obj.add(source)
			}
...
NOTE: Because these functions are chainable the FUNCTION is returned by get(), not the figures of that function.
...
*/
		...
		LocusZoom.ScaleFunctions.add(
			...
			function (parameters, value) {
				if (
					typeof value == 'undefined' ||
					parameters.categories.indexOf(value) == -1
				) {
					return parameters.null_value ? parameters.null_value : null
				} else {
					return parameters.values[
						parameters.categories.indexOf(value)
					]
				}
			}
		)
		...
		LocusZoom.ScaleFunctions.add(
			...
			function (parameters, input) {
				var breaks = parameters.breaks || []
				var values = parameters.values || []
				var nullval = parameters.null_value
				...
				if (+input <= parameters.breaks[0]) {
					...
					+input >= parameters.breaks[parameters.breaks.length - 1]
				) {
					...
					var normalized_input =
						(+input - breaks[upper_idx - 1]) /
						(breaks[upper_idx] - breaks[upper_idx - 1])
...
*/
		...
		LocusZoom.Dashboard = function (parent) {
			...
			if (
				!(parent instanceof LocusZoom.Plot) &&
				!(parent instanceof LocusZoom.Panel)
			) {
			...
			this.parent_plot =
				this.type == 'plot' ? this.parent : this.parent.parent
		...
		LocusZoom.Dashboard.prototype.initialize = function () {
			...
			if (Array.isArray(this.parent.layout.dashboard.components)) {
				this.parent.layout.dashboard.components.forEach(
					function (layout) {
						try {
							var component = LocusZoom.Dashboard.Components.get(
								layout.type,
								layout,
								this
							)
			...
			if (this.type == 'panel') {
				d3.select(this.parent.parent.svg.node().parentNode).on(
					'mouseover.' + this.id,
					function () {
		...
		LocusZoom.Dashboard.prototype.show = function () {
			if (!this.selector) {
				switch (this.type) {
					case 'plot':
						this.selector = d3
							.select(this.parent.svg.node().parentNode)
							.insert('div', ':first-child')
						break
					case 'panel':
						this.selector = d3
							.select(this.parent.parent.svg.node().parentNode)
							.insert(
								'div',
								'.lz-data_layer-tooltip, .lz-dashboard-menu, .lz-curtain'
							)
							.classed('lz-panel-dashboard', true)
						break
				}
				this.selector
					.classed('lz-dashboard', true)
					.classed('lz-' + this.type + '-dashboard', true)
					.attr('id', this.id)
			}
			this.components.forEach(function (component) {
				component.show()
...
*/
		...
		LocusZoom.Dashboard.Component.prototype.show = function () {
			...
			if (!this.selector) {
				...
				this.selector = this.parent.selector
					...
					.attr(
						'class',
						'lz-dashboard-' + this.layout.position + group_position
					)
		...
		LocusZoom.Dashboard.Components = (function () {
			...
			obj.set = function (name, component) {
				if (component) {
					if (typeof component != 'function') {
						...
						components[name].prototype =
							new LocusZoom.Dashboard.Component()
					}
				} else {
			...
			obj.add = function (name, component) {
				if (components[name]) {
					throw (
						'dashboard component already exists with name: ' + name
					)
				} else {
...
*/
		...
		LocusZoom.Dashboard.Component.Button = function (parent) {
			...
			this.setStatus = function (status) {
				if (
					typeof status != 'undefined' &&
					['', 'highlighted', 'disabled'].indexOf(status) !== -1
				) {
					this.status = status
				}
				return this.update()
			...
			this.update = function () {
				...
				this.selector
					...
					.on(
						'mouseover',
						this.status == 'disabled' ? null : this.onmouseover
					)
					.on(
						'mouseout',
						this.status == 'disabled' ? null : this.onmouseout
					)
					.on(
						'click',
						this.status == 'disabled' ? null : this.onclick
					)
					.text(this.text)
				this.menu.update()
				this.postUpdate()
			...
			this.menu = {
				...
				show: function () {
					if (!this.menu.outer_selector) {
						this.menu.outer_selector = d3
							.select(this.parent_plot.svg.node().parentNode)
							.append('div')
							.attr(
								'class',
								'lz-dashboard-menu lz-dashboard-menu-' +
									this.color
							)
							.attr(
								'id',
				...
				position: function () {
					...
					var dashboard_client_rect = this.parent_dashboard.selector
						.node()
						.getBoundingClientRect()
					var button_client_rect = this.selector
						.node()
						.getBoundingClientRect()
					var menu_client_rect = this.menu.outer_selector
						.node()
						.getBoundingClientRect()
					var total_content_height =
						this.menu.inner_selector.node().scrollHeight
					var top = 0
					var left = 0
					if (this.parent_dashboard.type == 'panel') {
						top =
							page_origin.y +
							dashboard_client_rect.height +
							2 * padding
						left = Math.max(
							page_origin.x +
								this.parent_svg.layout.width -
								menu_client_rect.width -
								padding,
							page_origin.x + padding
						)
					} else {
						top =
							button_client_rect.bottom +
							page_scroll_top +
							padding -
							container_offset.top
						left = Math.max(
							button_client_rect.left +
								button_client_rect.width -
								menu_client_rect.width -
								container_offset.left,
							page_origin.x + padding
						)
					}
					var base_max_width = Math.max(
						this.parent_svg.layout.width -
							2 * padding -
							scrollbar_padding,
						scrollbar_padding
					...
					var base_max_height = Math.max(
						this.parent_svg.layout.height -
							10 * padding -
							menu_height_padding,
						menu_height_padding
					)
					...
					this.menu.inner_selector.style({
						'max-width': content_max_width.toString() + 'px',
					})
					this.menu.inner_selector.node().scrollTop =
						this.menu.scroll_position
					return this.menu
		...
		LocusZoom.Dashboard.Components.add('download', function (layout) {
			...
			for (var stylesheet in Object.keys(document.styleSheets)) {
				if (
					document.styleSheets[stylesheet].href != null &&
					document.styleSheets[stylesheet].href.indexOf(
						'locuszoom.css'
					) != -1
				) {
					LocusZoom.createCORSPromise(
						'GET',
						document.styleSheets[stylesheet].href
					).then(
						function (response) {
			...
			this.generateBase64SVG = function () {
				return Q.fcall(
					function () {
						...
						var container = this.parent.selector
							.append('div')
							.style('display', 'none')
						...
						var initial_html = d3
							.select(container.select('svg').node().parentNode)
							.html()
						var style_def =
						...
						initial_html =
							initial_html.slice(0, insert_at) +
							style_def +
							initial_html.slice(insert_at)
						// Delete the container node
						container.remove()
						// Base64-encode the string and return it
						return btoa(
							encodeURIComponent(initial_html).replace(
								/%([0-9A-F]{2})/g,
								function (match, p1) {
					...
					.setOnclick(
						function () {
							if (
								...
								d3.select(
									panel.parent.svg.node().parentNode
								).on(
									'mouseover.' +
										panel.getBaseId() +
										'.dashboard',
		...
		// Move Panel Down
		LocusZoom.Dashboard.Components.add(
			'move_panel_down',
			function (layout) {
				LocusZoom.Dashboard.Component.apply(this, arguments)
				this.update = function () {
		...
		LocusZoom.Dashboard.Components.add('shift_region', function (layout) {
			...
			this.update = function () {
				...
				this.button = new LocusZoom.Dashboard.Component.Button(this)
					.setColor(layout.color)
					.setText(layout.button_html)
					.setTitle(layout.button_title)
					.setOnclick(
						function () {
							this.parent_plot.applyState({
								start: Math.max(
									this.parent_plot.state.start + layout.step,
									1
								),
		...
		LocusZoom.Dashboard.Components.add('zoom_region', function (layout) {
			...
			if (typeof layout.button_title != 'string') {
				layout.button_title =
					'Zoom region ' +
					(layout.step > 0 ? 'out' : 'in') +
					' by ' +
					(Math.abs(layout.step) * 100).toFixed(1) +
					'%'
			}
			this.update = function () {
				if (this.button) {
					var can_zoom = true
					var current_region_scale =
						this.parent_plot.state.end -
						this.parent_plot.state.start
					if (
						layout.step > 0 &&
						!isNaN(this.parent_plot.layout.max_region_scale) &&
						current_region_scale >=
				...
				this.button = new LocusZoom.Dashboard.Component.Button(this)
					...
					.setOnclick(
						function () {
							...
							var new_region_scale =
								current_region_scale * zoom_factor
							if (
								!isNaN(this.parent_plot.layout.max_region_scale)
							) {
								new_region_scale = Math.min(
									new_region_scale,
									this.parent_plot.layout.max_region_scale
								)
							}
							if (
								...
								new_region_scale = Math.max(
									new_region_scale,
									this.parent_plot.layout.min_region_scale
								)
							}
							var delta = Math.floor(
								(new_region_scale - current_region_scale) / 2
							)
							this.parent_plot.applyState({
								start: Math.max(
									this.parent_plot.state.start - delta,
									1
								),
					...
					this.parent_plot.CovariatesModel = {
						...
						add: function (element_reference) {
							...
							var element = JSON.parse(
								JSON.stringify(element_reference)
							)
							if (
								typeof element_reference == 'object' &&
								typeof element.html != 'string'
							) {
								element.html =
									typeof element_reference.toHTML ==
									'function'
										? element_reference.toHTML()
										: element_reference.toString()
							}
							// Check if the element is already in the model covariates array and return if it is.
							for (
								var i = 0;
								i < this.state.model.covariates.length;
								i++
							) {
								if (
									JSON.stringify(
										this.state.model.covariates[i]
						...
						removeByIdx: function (idx) {
							if (
								typeof this.state.model.covariates[idx] ==
								'undefined'
							) {
								throw (
				...
				this.update = function () {
					...
					this.button.menu.setPopulate(
						function () {
							...
							if (
								typeof this.parent_plot.state.model.html !=
								'undefined'
							) {
								selector
									.append('div')
							...
							if (
								...
								this.parent_plot.state.model.covariates.forEach(
									function (covariate, idx) {
										var html =
											typeof covariate == 'object' &&
											typeof covariate.html == 'string'
												? covariate.html
												: covariate.toString()
		...
		LocusZoom.Dashboard.Components.add(
			...
			function (layout) {
				...
				this.update = function () {
					var data_layer =
						this.parent_panel.data_layers[layout.data_layer_id]
					var text = data_layer.layout.split_tracks
						? 'Merge Tracks'
						: 'Split Tracks'
					if (this.button) {
						...
						this.button = new LocusZoom.Dashboard.Component.Button(
							this
						)
							.setColor(layout.color)
							.setText(text)
							...
							.setOnclick(
								function () {
									...
									var timeout = data_layer.layout.transition
										? +data_layer.layout.transition
												.duration || 0
										: 0
		...
		LocusZoom.Dashboard.Components.add('toggle_legend', function (layout) {
			LocusZoom.Dashboard.Component.apply(this, arguments)
			this.update = function () {
				var text = this.parent_panel.legend.layout.hidden
					? 'Show Legend'
					: 'Hide Legend'
				if (this.button) {
					...
					function () {
						...
						this.parent_panel.data_layer_ids_by_z_index
							...
							.forEach(
								function (id, idx) {
									var data_layer =
										this.parent_panel.data_layers[id]
									var name =
										typeof data_layer.layout.name !=
										'string'
											? data_layer.id
											: data_layer.layout.name
									var row = table.append('tr')
									// Layer name
									row.append('td').html(name)
									// Status toggle buttons
									layout.statuses.forEach(
										function (status_adj) {
											var status_idx =
												LocusZoom.DataLayer.Statuses.adjectives.indexOf(
													status_adj
												)
											var status_verb =
												LocusZoom.DataLayer.Statuses
										...
										.on(
											...
											function () {
												if (
													...
													data_layer.parent.removeDataLayer(
														id
													)
												}
...
*/
		...
		LocusZoom.Legend = function (parent) {
			...
			this.parent.layout.legend = LocusZoom.Layouts.merge(
			...
			this.parent.data_layer_ids_by_z_index
				...
				.forEach(
					function (id) {
						if (
							Array.isArray(
								this.parent.data_layers[id].layout.legend
							)
						) {
							this.parent.data_layers[id].layout.legend.forEach(
								function (element) {
									var selector = this.elements_group
										.append('g')
										.attr(
											'transform',
											'translate(' + x + ',' + y + ')'
									...
									if (this.layout.orientation == 'vertical') {
										...
										var right_x =
											this.layout.origin.x + x + bcr.width
										if (
											x > padding &&
											right_x > this.parent.layout.width
										) {
											y += line_height
											x = padding
											selector.attr(
		...
		LocusZoom.DataSources.prototype.addSource = function (ns, x) {
			console.warn(
				'Warning: .addSource() is depricated. Use .add() instead'
			)
		...
		LocusZoom.DataSources.prototype.getSource = function (ns) {
			console.warn(
				'Warning: .getSource() is depricated. Use .get() instead'
			)
			return this.get(ns)
		}
		...
		LocusZoom.DataSources.prototype.removeSource = function (ns) {
			console.warn(
				'Warning: .removeSource() is depricated. Use .remove() instead'
			)
		...
		LocusZoom.Data.Field = function (field) {
				...
				this.transformations.forEach(
					function (transform, i) {
						this.transformations[i] =
							LocusZoom.TransformationFunctions.get(transform)
					}.bind(this)
				)
			}
			...
			this.resolve = function (d) {
				if (typeof d[this.full_name] == 'undefined') {
					var val = null
					if (
						typeof d[this.namespace + ':' + this.name] !=
						'undefined'
					) {
...
*/
		...
		LocusZoom.Data.Source.prototype.getCacheKey = function (
			state,
			chain,
			fields
		) {
		...
		LocusZoom.Data.Source.prototype.fetchRequest = function (
			state,
			chain,
			fields
		) {
		...
		LocusZoom.Data.Source.prototype.getRequest = function (
			state,
			chain,
			fields
		) {
		...
		LocusZoom.Data.Source.prototype.getData = function (
			state,
			fields,
			outnames,
			trans
			...
			return function (chain) {
				return this.getRequest(state, chain, fields).then(
					function (resp) {
		...
		LocusZoom.Data.Source.prototype.parseResponse = function (
			resp,
			chain,
			fields,
			outnames,
		...
		LocusZoom.Data.Source.prototype.parseArraysToObjects = function (
			x,
			fields,
			outnames,
			trans
		...
		LocusZoom.Data.Source.prototype.parseObjectsToObjects = function (
			x,
			fields,
			outnames,
			trans
		...
		LocusZoom.Data.Source.prototype.parseData = function (
			x,
			fields,
			outnames,
			trans
...
*/
		LocusZoom.Data.AssociationSource = LocusZoom.Data.Source.extend(
			function (init) {
				this.parseInit(init)
			},
			'AssociationLZ'
		...
		LocusZoom.Data.AssociationSource.prototype.preGetData = function (
			state,
			fields,
			outnames,
			trans
		...
		LocusZoom.Data.AssociationSource.prototype.getURL = function (
			state,
			chain,
			fields
		) {
...
*/
		...
		LocusZoom.Data.LDSource.prototype.preGetData = function (
			state,
			fields
		) {
			if (fields.length > 1) {
				if (fields.length != 2 || fields.indexOf('isrefvar') == -1) {
					throw (
						'LD does not know how to get all fields: ' +
						fields.join(', ')
					)
				}
			}
		...
		LocusZoom.Data.LDSource.prototype.findMergeFields = function (chain) {
			...
			if (chain && chain.body && chain.body.length > 0) {
				...
				dataFields.id =
					dataFields.id ||
					nameMatch(/\bvariant\b/) ||
					nameMatch(/\bid\b/)
				dataFields.position =
					dataFields.position ||
					nameMatch(/\bposition\b/i, /\bpos\b/i)
				dataFields.pvalue =
					dataFields.pvalue ||
					nameMatch(/\bpvalue\b/i, /\blog_pvalue\b/i)
				dataFields._names_ = names
			}
			return dataFields
		}

		LocusZoom.Data.LDSource.prototype.findRequestedFields = function (
			fields,
			outnames
		) {
			var obj = {}
		...
		LocusZoom.Data.LDSource.prototype.getURL = function (
			state,
			chain,
			fields
		) {
			...
			if (refVar == 'best') {
				...
				refVar =
					chain.body[findExtremeValue(chain.body, keys.pvalue)][
						keys.id
					]
			}
			if (!chain.header) {
		...
		LocusZoom.Data.LDSource.prototype.parseResponse = function (
			resp,
			chain,
			fields,
			outnames
			...
			if (reqFields.isrefvarin && chain.header.ldrefvar) {
				tagRefVariant(
					chain.body,
					chain.header.ldrefvar,
					keys.id,
					reqFields.isrefvarout
				)
			}
			return chain
		}
...
*/
		LocusZoom.Data.GeneSource = LocusZoom.Data.Source.extend(function (
			init
		) {
			this.parseInit(init)
		},
		...
		LocusZoom.Data.GeneSource.prototype.getURL = function (
			state,
			chain,
			fields
		) {
			var source =
				state.source || chain.header.source || this.params.source || 2
			return (
				this.url +
				'?filter=source in ' +
				source +
		...
		LocusZoom.Data.GeneSource.prototype.parseResponse = function (
			resp,
			chain,
			fields,
			outnames
...
*/
		LocusZoom.Data.GeneConstraintSource = LocusZoom.Data.Source.extend(
			function (init) {
				this.parseInit(init)
			},
			'GeneConstraintLZ'
		...
		LocusZoom.Data.GeneConstraintSource.prototype.getCacheKey = function (
			state,
			chain,
			fields
		) {
		...
		LocusZoom.Data.GeneConstraintSource.prototype.fetchRequest = function (
			state,
			chain,
			fields
		) {
		...
		LocusZoom.Data.GeneConstraintSource.prototype.parseResponse = function (
			resp,
			chain,
			fields,
			outnames
			...
			// Loop through the array of genes in the body and match each to a figures from the contraints request
...
*/
		LocusZoom.Data.RecombinationRateSource = LocusZoom.Data.Source.extend(
			function (init) {
				this.parseInit(init)
			},
			'RecombLZ'
		...
		LocusZoom.Data.RecombinationRateSource.prototype.getURL = function (
			state,
			chain,
			fields
		) {
...
*/
		...
		LocusZoom.Data.IntervalSource = LocusZoom.Data.Source.extend(function (
			init
		) {
			this.parseInit(init)
		},
		...
		LocusZoom.Data.IntervalSource.prototype.getURL = function (
			state,
			chain,
			fields
		) {
			var source =
				state.bedtracksource ||
				chain.header.bedtracksource ||
				this.params.source ||
				16
			return (
				this.url +
...
*/
		LocusZoom.Data.StaticSource = LocusZoom.Data.Source.extend(function (
			data
		) {
			this._data = data
		},
		...
		LocusZoom.Data.StaticSource.prototype.getRequest = function (
			state,
			chain,
			fields
		) {
...
*/
		LocusZoom.Data.PheWASSource = LocusZoom.Data.Source.extend(function (
			init
		) {
			this.parseInit(init)
		},
		'PheWASLZ')
		LocusZoom.Data.PheWASSource.prototype.getURL = function (
			state,
			chain,
			fields
		) {
		...
		LocusZoom.Data.PheWASSource.prototype.parseResponse = function (
			resp,
			chain,
			fields,
			outnames,
...
*/
		...
		LocusZoom.Plot = function (id, datasource, layout) {
			...
			this.on = function (event, hook) {
				if (
					typeof 'event' != 'string' ||
					!Array.isArray(this.event_hooks[event])
				) {
					throw (
			...
			this.getPageOrigin = function () {
				var bounding_client_rect = this.svg
					.node()
					.getBoundingClientRect()
				var x_offset =
				...
				while (container.parentNode != null) {
					...
					if (
						container != document &&
						d3.select(container).style('position') != 'static'
					) {
			...
			this.canInteract = function (panel_id) {
				...
				if (panel_id) {
					return (
						(typeof this.interaction.panel_id == 'undefined' ||
							this.interaction.panel_id == panel_id) &&
						!this.loading_data
					)
				} else {
					return !(
		...
		LocusZoom.Plot.prototype.sumProportional = function (dimension) {
			...
			for (var id in this.panels) {
				...
				if (!this.panels[id].layout['proportional_' + dimension]) {
					this.panels[id].layout['proportional_' + dimension] =
		...
		LocusZoom.Plot.prototype.initializeLayout = function () {
			...
			if (this.layout.responsive_resize) {
				this.window_onresize = d3.select(window).on(
					'resize.lz-' + this.id,
					function () {
						this.rescaleSVG()
					}.bind(this)
...
*/
		LocusZoom.Plot.prototype.setDimensions = function (width, height) {
			...
			for (id in this.panels) {
				min_width = Math.max(
					min_width,
					this.panels[id].layout.min_width
				)
				if (
					parseFloat(this.panels[id].layout.min_height) > 0 &&
					parseFloat(this.panels[id].layout.proportional_height) > 0
				) {
					min_height = Math.max(
						min_height,
						this.panels[id].layout.min_height /
							this.panels[id].layout.proportional_height
					)
				}
			}
			...
			if (!isNaN(width) && width >= 0 && !isNaN(height) && height >= 0) {
				...
				this.panel_ids_by_y_index.forEach(
					function (panel_id) {
						...
						var panel_height =
							this.panels[panel_id].layout.proportional_height *
							this.layout.height
						...
						this.panels[panel_id].layout.proportional_origin.y =
							y_offset / this.layout.height
		...
		LocusZoom.Plot.prototype.addPanel = function (layout) {
			...
			if (
				panel.layout.y_index != null &&
				!isNaN(panel.layout.y_index) &&
				...
				if (panel.layout.y_index < 0) {
					panel.layout.y_index = Math.max(
						this.panel_ids_by_y_index.length + panel.layout.y_index,
						0
					)
				}
				this.panel_ids_by_y_index.splice(
					panel.layout.y_index,
					0,
					panel.id
				)
			...
			if (layout_idx == null) {
				layout_idx =
					this.layout.panels.push(this.panels[panel.id].layout) - 1
			}
		...
		LocusZoom.Plot.prototype.removePanel = function (id) {
			...
			this.panels[id].data_layer_ids_by_z_index.forEach(
				function (dlid) {
			...
			this.panel_ids_by_y_index.splice(
				this.panel_ids_by_y_index.indexOf(id),
				1
			)
			this.applyPanelYIndexesToPanelLayouts()
...
*/
		LocusZoom.Plot.prototype.positionPanels = function () {
			...
			for (id in this.panels) {
				if (this.panels[id].layout.proportional_height == null) {
					this.panels[id].layout.proportional_height =
						this.panels[id].layout.height / this.layout.height
				}
				...
				if (this.panels[id].layout.interaction.x_linked) {
					x_linked_margins.left = Math.max(
						x_linked_margins.left,
						this.panels[id].layout.margin.left
					)
					x_linked_margins.right = Math.max(
						x_linked_margins.right,
						this.panels[id].layout.margin.right
					)
				}
			}
				...
				function (panel_id) {
					...
					if (this.panels[panel_id].layout.interaction.x_linked) {
						var delta =
							Math.max(
								x_linked_margins.left -
									this.panels[panel_id].layout.margin.left,
								0
							) +
							Math.max(
								x_linked_margins.right -
									this.panels[panel_id].layout.margin.right,
								0
							)
						...
						this.panels[panel_id].layout.margin.right =
							x_linked_margins.right
						this.panels[panel_id].layout.cliparea.origin.x =
							x_linked_margins.left
					}
				}.bind(this)
			)
			...
			this.panel_ids_by_y_index.forEach(
				function (panel_id) {
					this.panels[panel_id].layout.proportional_origin.y =
						this.panels[panel_id].layout.origin.y /
						calculated_plot_height
				}.bind(this)
			)
			...
			this.panel_ids_by_y_index.forEach(
				function (panel_id) {
					this.panels[panel_id].setDimensions(
						this.layout.width *
							this.panels[panel_id].layout.proportional_width,
						this.layout.height *
							this.panels[panel_id].layout.proportional_height
					)
				}.bind(this)
		...
		LocusZoom.Plot.prototype.initialize = function () {
			...
			if (this.layout.responsive_resize) {
				d3.select(this.container).classed(
			...
			if (this.layout.mouse_guide) {
				var mouse_guide_svg = this.svg
					.append('g')
					.attr('class', 'lz-mouse_guide')
					.attr('id', this.id + '.mouse_guide')
				var mouse_guide_vertical_svg = mouse_guide_svg
					.append('rect')
					.attr('class', 'lz-mouse_guide-vertical')
					.attr('x', -1)
				var mouse_guide_horizontal_svg = mouse_guide_svg
					.append('rect')
			...
			this.panel_boundaries = {
				...
				show: function () {
					...
					if (!this.showing && !this.parent.curtain.showing) {
								...
								panel_resize_drag.on(
									...
									function () {
										...
										var this_panel =
											this.parent.panels[
												this.parent
													.panel_ids_by_y_index[
													panel_idx
												]
											]
										var original_panel_height =
											this_panel.layout.height
										this_panel.setDimensions(
											this_panel.layout.width,
											this_panel.layout.height +
												d3.event.dy
										)
										var panel_height_change =
											this_panel.layout.height -
											original_panel_height
										var new_calculated_plot_height =
											this.parent.layout.height +
											panel_height_change
										...
										this.parent.panel_ids_by_y_index.forEach(
											function (
												...
												loop_panel.layout.proportional_height =
													loop_panel.layout.height /
													new_calculated_plot_height
												if (
													loop_panel_idx > panel_idx
												) {
													loop_panel.setOrigin(
														loop_panel.layout.origin
															.x,
														loop_panel.layout.origin
															.y +
															panel_height_change
						...
						corner_drag.on(
							...
							function () {
								this.setDimensions(
									this.layout.width + d3.event.dx,
									this.layout.height + d3.event.dy
								)
							}.bind(this.parent)
						)
						corner_selector.call(corner_drag)
						this.parent.panel_boundaries.corner_selector =
							corner_selector
					}
					return this.position()
				},
				position: function () {
					...
					this.selectors.forEach(
						function (selector, panel_idx) {
							var panel_page_origin =
								this.parent.panels[
									this.parent.panel_ids_by_y_index[panel_idx]
								].getPageOrigin()
			...
			if (this.layout.panel_boundaries) {
				d3.select(this.svg.node().parentNode).on(
			...
			if (this.layout.mouse_guide) {
				...
				this.svg
					.on(
						'mouseout' + namespace + '-mouse_guide',
						mouseout_mouse_guide
					)
					.on(
						'touchleave' + namespace + '-mouse_guide',
						mouseout_mouse_guide
					)
					.on(
						'mousemove' + namespace + '-mouse_guide',
						mousemove_mouse_guide
					)
			}
			...
			var mousemove = function () {
				if (this.interaction.dragging) {
					...
					this.interaction.dragging.dragged_x =
						coords[0] - this.interaction.dragging.start_x
					this.interaction.dragging.dragged_y =
						coords[1] - this.interaction.dragging.start_y
			...
			var width = client_rect.width
				? client_rect.width
				: this.layout.width
			var height = client_rect.height
				? client_rect.height
				: this.layout.height
					...
					function () {
						...
						this.panel_ids_by_y_index.forEach(
							function (panel_id) {
								...
								panel.data_layer_ids_by_z_index.forEach(
									function (data_layer_id) {
										var data_layer =
											this.data_layers[data_layer_id]
										var state_id =
											panel_id + '.' + data_layer_id
		...
		LocusZoom.Plot.prototype.startDrag = function (panel, method) {
			...
			if (
				!(panel instanceof LocusZoom.Panel) ||
				!axis ||
				!this.canInteract()
			) {
		...
		LocusZoom.Plot.prototype.stopDrag = function () {
			...
			var overrideAxisLayout = function (axis, axis_number, extent) {
				panel.data_layer_ids_by_z_index.forEach(function (id) {
					if (
						panel.data_layers[id].layout[axis + '_axis'].axis ==
						axis_number
					) {
						panel.data_layers[id].layout[axis + '_axis'].floor =
							extent[0]
						panel.data_layers[id].layout[axis + '_axis'].ceiling =
							extent[1]
						delete panel.data_layers[id].layout[axis + '_axis']
							.lower_buffer
						delete panel.data_layers[id].layout[axis + '_axis']
							.upper_buffer
						delete panel.data_layers[id].layout[axis + '_axis']
...
*/
		...
		LocusZoom.Panel = function (layout, parent) {
			...
			if (typeof layout.id !== 'string' || !layout.id.length) {
				if (!this.parent) {
					layout.id =
						'p' + Math.floor(Math.random() * Math.pow(10, 8))
				} else {
					var id = null
					var generateID = function () {
						...
						if (
							id == null ||
							typeof this.parent.panels[id] != 'undefined'
						) {
							id = generateID()
						}
					}.bind(this)
		...
		LocusZoom.Panel.prototype.initializeLayout = function () {
			...
			if (
				this.layout.width == 0 &&
				this.layout.proportional_width == null
			) {
			...
			if (
				this.layout.height == 0 &&
				this.layout.proportional_height == null
			) {
			...
			;['x', 'y1', 'y2'].forEach(
				function (axis) {
					if (
						!Object.keys(this.layout.axes[axis]).length ||
						...
						this.layout.axes[axis].label =
							this.layout.axes[axis].label || null
						this.layout.axes[axis].label_function =
							this.layout.axes[axis].label_function || null
					}
				}.bind(this)
			)
		...
		LocusZoom.Panel.prototype.setMargin = function (
			top,
			right,
			bottom,
			left
		...
		LocusZoom.Panel.prototype.setTitle = function (title) {
			...
			if (typeof title == 'string') {
				...
				this.layout.title = LocusZoom.Layouts.merge(
					title,
					this.layout.title
				)
			}
		...
		LocusZoom.Panel.prototype.initialize = function () {
			...
			var clipPath = this.svg.container
				.append('clipPath')
				.attr('id', this.getBaseId() + '.clip')
			this.svg.clipRect = clipPath
				.append('rect')
				.attr('width', this.layout.width)
				.attr('height', this.layout.height)
			...
			this.inner_border = this.svg.group
				...
				.on(
					...
					function () {
						if (
							this.layout.background_click == 'clear_selections'
						) {
		...
		LocusZoom.Panel.prototype.getLinkedPanelIds = function (axis) {
			...
			this.parent.panel_ids_by_y_index.forEach(
				function (panel_id) {
					if (
						panel_id != this.id &&
						this.parent.panels[panel_id].layout.interaction[
							axis + '_linked'
						]
					) {
		...
		LocusZoom.Panel.prototype.addDataLayer = function (layout) {
			...
			if (
				typeof layout.y_axis == 'object' &&
				(typeof layout.y_axis.axis == 'undefined' ||
					[1, 2].indexOf(layout.y_axis.axis) == -1)
			) {
			...
			if (
				data_layer.layout.z_index != null &&
				!isNaN(data_layer.layout.z_index) &&
				...
				if (data_layer.layout.z_index < 0) {
					data_layer.layout.z_index = Math.max(
						this.data_layer_ids_by_z_index.length +
							data_layer.layout.z_index,
						0
					)
				}
			...
			if (layout_idx == null) {
				layout_idx =
					this.layout.data_layers.push(
						this.data_layers[data_layer.id].layout
					) - 1
			}
		...
		LocusZoom.Panel.prototype.removeDataLayer = function (id) {
			...
			this.data_layer_ids_by_z_index.splice(
				this.data_layer_ids_by_z_index.indexOf(id),
				1
			)
			...
			this.applyDataLayerZIndexesToDataLayerLayouts()
			this.layout.data_layers.forEach(
				function (data_layer_layout, idx) {
		...
		LocusZoom.Panel.prototype.generateExtents = function () {
			...
			for (var id in this.data_layers) {
				...
				if (
					data_layer.layout.x_axis &&
					!data_layer.layout.x_axis.decoupled
				) {
					this.x_extent = d3.extent(
						(this.x_extent || []).concat(
							data_layer.getAxisExtent('x')
						)
					)
				}
				...
				if (
					...
					!data_layer.layout.y_axis.decoupled
				) {
					var y_axis = 'y' + data_layer.layout.y_axis.axis
					this[y_axis + '_extent'] = d3.extent(
						(this[y_axis + '_extent'] || []).concat(
							data_layer.getAxisExtent('y')
						)
					)
				}
			}
		...
		LocusZoom.Panel.prototype.render = function () {
			...
			if (this.x_extent) {
				...
				if (this.layout.axes.x.range) {
					base_x_range.start =
						this.layout.axes.x.range.start || base_x_range.start
					base_x_range.end =
						this.layout.axes.x.range.end || base_x_range.end
				}
			...
			if (this.y1_extent) {
				...
				if (this.layout.axes.y1.range) {
					base_y1_range.start =
						this.layout.axes.y1.range.start || base_y1_range.start
					base_y1_range.end =
						this.layout.axes.y1.range.end || base_y1_range.end
				}
			...
			if (this.y2_extent) {
				...
				if (this.layout.axes.y2.range) {
					base_y2_range.start =
						this.layout.axes.y2.range.start || base_y2_range.start
					base_y2_range.end =
						this.layout.axes.y2.range.end || base_y2_range.end
				}
			...
			if (
				...
				if (
					...
					typeof this.x_scale == 'function'
				) {
					var current_extent_size = Math.abs(
						this.x_extent[1] - this.x_extent[0]
					)
					var current_scaled_extent_size =
						Math.round(this.x_scale.invert(ranges.x_shifted[1])) -
						Math.round(this.x_scale.invert(ranges.x_shifted[0]))
					var zoom_factor = this.parent.interaction.zooming.scale
					var potential_extent_size = Math.floor(
						current_scaled_extent_size * (1 / zoom_factor)
					)
					if (
						zoom_factor < 1 &&
						!isNaN(this.parent.layout.max_region_scale)
					) {
						zoom_factor =
							1 /
							(Math.min(
								potential_extent_size,
								this.parent.layout.max_region_scale
							) /
								current_scaled_extent_size)
					} else if (
						zoom_factor > 1 &&
						...
						zoom_factor =
							...
							(Math.max(
								potential_extent_size,
								this.parent.layout.min_region_scale
							) /
								current_scaled_extent_size)
					}
					var new_extent_size = Math.floor(
						current_extent_size * zoom_factor
					)
					anchor =
					...
					var new_x_extent_start = Math.max(
						Math.floor(
							this.x_scale.invert(ranges.x_shifted[0]) -
								(new_extent_size - current_scaled_extent_size) *
									offset_ratio
						),
						1
					)
					...
					switch (this.parent.interaction.dragging.method) {
						case 'background':
							ranges.x_shifted[0] =
								0 + this.parent.interaction.dragging.dragged_x
							ranges.x_shifted[1] =
						...
						case 'x_tick':
							if (d3.event && d3.event.shiftKey) {
								...
								scalar = constrain(
									anchor /
										(anchor +
											this.parent.interaction.dragging
												.dragged_x),
									3
								)
								ranges.x_shifted[0] = 0
								ranges.x_shifted[1] = Math.max(
									this.layout.cliparea.width * (1 / scalar),
									1
								)
							}
							break
						...
						case 'y2_tick':
							...
							if (d3.event && d3.event.shiftKey) {
								...
								ranges[y_shifted][1] =
									this.layout.cliparea.height -
									this.layout.cliparea.height * (1 / scalar)
							}
					}
				}
			}
			...
			;['x', 'y1', 'y2'].forEach(
				function (axis) {
					...
					if (this.layout.axes[axis].ticks) {
						...
						this[axis + '_ticks'] = LocusZoom.prettyTicks(
							this[axis + '_extent'],
							'both'
						)
					}
				...
				var zoom_handler = function () {
					...
					if (!d3.event.shiftKey) {
						if (this.parent.canInteract(this.id)) {
							this.loader
								.show(
									'Press <tt>[SHIFT]</tt> while scrolling to zoom'
								)
								.hide(1000)
						}
						return
		...
		LocusZoom.Panel.prototype.renderAxis = function (axis) {
			...
			if (ticksAreAllNumbers) {
				...
				this[axis + '_axis'].tickValues(ticks).tickFormat(
					function (t, i) {
			...
			if (!ticksAreAllNumbers) {
				var tick_selector = d3.selectAll(
					'g#' +
			...
			if (label != null) {
				...
				if (axis_params[axis].label_rotate != null) {
					this.svg[axis + '_axis_label'].attr(
						'transform',
						'rotate(' +
							axis_params[axis].label_rotate +
							' ' +
							axis_params[axis].label_x +
							',' +
							axis_params[axis].label_y +
							')'
					)
				}
			}
			...
			;['x', 'y1', 'y2'].forEach(
				function (axis) {
					if (
						this.layout.interaction[
							'drag_' + axis + '_ticks_to_scale'
						]
					) {
						var namespace =
							'.' +
						...
						var tick_mouseover = function () {
							if (
								typeof d3.select(this).node().focus ==
								'function'
							) {
						...
						this.svg.container
							.selectAll('.lz-axis.lz-' + axis + ' .tick text')
		...
		LocusZoom.Panel.prototype.setElementStatusByFilters = function (
			status,
			toggle,
			filters,
			exclusive
			...
			this.data_layer_ids_by_z_index.forEach(
				function (id) {
					this.data_layers[id].setElementStatusByFilters(
						status,
						toggle,
						filters,
						exclusive
		...
		LocusZoom.Panel.prototype.setAllElementStatus = function (
			status,
			toggle
		) {
			this.data_layer_ids_by_z_index.forEach(
		...
		LocusZoom.DataLayer.Statuses.verbs.forEach(function (verb, idx) {
			...
			var antiverb = 'un' + verb
			// Set/unset status for arbitrarily many elements given a set of filters
			LocusZoom.Panel.prototype[verb + 'ElementsByFilters'] = function (
				filters,
				exclusive
			) {
				if (typeof exclusive == 'undefined') {
			...
			LocusZoom.Panel.prototype[antiverb + 'ElementsByFilters'] =
				function (filters, exclusive) {
					if (typeof exclusive == 'undefined') {
						exclusive = false
					} else {
