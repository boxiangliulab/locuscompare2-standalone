    ...
    LocusZoom.validateState = function (new_state, layout) {
      ...
      if (
        ...
        new_state.start = Math.max(
          attempted_midpoint - Math.floor(layout.min_region_scale / 2),
          1
        )
      ...
      if (
        ...
        new_state.start = Math.max(
          attempted_midpoint - Math.floor(layout.max_region_scale / 2),
          1
        )
    ...
    LocusZoom.parseFields = function (data, html) {
      ...
      while (html.length > 0) {
        ...
        if (!m) {
          ...
          console.error(
            'Error tokenizing tooltip when remaining template is: ' + html
          )
          html = html.slice(m[0].length)
        }
      }
      ...
      var resolve = function (variable) {
        if (!resolve.cache.hasOwnProperty(variable)) {
          resolve.cache[variable] = new LocusZoom.Data.Field(variable).resolve(
            data
          )
        }
      ...
      var render_node = function (node) {
        if (node.text) {
          ...
          try {
            ...
            console.error(
              'Error while processing variable ' + JSON.stringify(node.variable)
            )
          }
            ...
            if (typeof element == 'string') {
              ...
              while ((match = re.exec(element)) !== null) {
                ...
                if (
                  ...
                  resolved_namespace =
                    namespace[key] + (namespace[key].length ? ':' : '')
                }
...
*/
    ...
    LocusZoom.DataLayer.Statuses.verbs.forEach(function (verb, idx) {
      ...
      LocusZoom.DataLayer.prototype[verb + 'ElementsByFilters'] = function (
        filters,
        exclusive
      ) {
        if (typeof exclusive == 'undefined') {
      ...
      LocusZoom.DataLayer.prototype[antiverb + 'ElementsByFilters'] = function (
        filters,
        exclusive
      ) {
        if (typeof exclusive == 'undefined') {
      ...
      Object.keys(this.layout.behaviors).forEach(
        function (directive) {
          ...
          selection.on(
            ...
            this.executeBehaviors(directive, this.layout.behaviors[directive])
          )
        }.bind(this)
      )
    }
            ...
            switch (behavior.action) {
              ...
              case 'toggle':
                var current_status_boolean =
                  this.state[this.state_id][behavior.status].indexOf(
    ...
    LocusZoom.DataLayers.add('scatter', function (layout) {
      ...
      this.positionTooltip = function (id) {
        ...
        var data_layer_height =
          ...
          (this.parent.layout.margin.top + this.parent.layout.margin.bottom)
        var data_layer_width =
          this.parent.layout.width -
          (this.parent.layout.margin.left + this.parent.layout.margin.right)
        if (this.layout.tooltip_positioning == 'vertical') {
          ...
          if (
            ...
            top =
              page_origin.y +
              y_center -
              (offset + tooltip_box.height + stroke_width + arrow_width)
            arrow_type = 'down'
          ...
          if (x_center <= this.parent.layout.width / 2) {
            left =
              page_origin.x + x_center + offset + arrow_width + stroke_width
            arrow_type = 'left'
      ...
      this.flip_labels = function () {
        ...
        data_layer.label_texts.each(function (d, i) {
          ...
          data_layer.label_texts.each(function () {
            ...
            var collision =
              abound.left < bbound.left + bbound.width + 2 * spacing &&
      ...
      this.positionTooltip = function (id) {
        ...
        if (Math.abs(dd.slope) > 1) {
          ...
          if (dd.display.y - tooltip_box.height / 2 <= 0) {
            // Too close to the top, push it down
            top =
              page_origin.y + dd.display.y - 1.5 * arrow_width - border_radius
            arrow_top = border_radius
          } else if (
          ...
          arrow_left =
            tooltip_box.width / 2 - arrow_width + offset_left - offset_right
          arrow_left = Math.min(
...
*/
    ...
    LocusZoom.DataLayers.add('genes', function (layout) {
        ...
        this.data.map(
          function (d, g) {
            ...
            this.data[g].transcript_id =
              this.data[g].transcripts[this.transcript_idx].transcript_id
            ...
            this.data[g].display_range.label_width = this.getLabelWidth(
            ...
            if (
              ...
              if (d.start < this.state.start) {
                ...
                if (
                  ...
                  this.data[g].display_range.start = this.parent.x_scale(
                    this.state.start
                  )
                  this.data[g].display_range.end =
                    this.data[g].display_range.start +
                  ...
                  this.data[g].display_range.end = this.parent.x_scale(
                    this.state.end
                  )
                  this.data[g].display_range.start =
                    this.data[g].display_range.end -
              ...
              this.gene_track_index[potential_track].map(
                function (placed_gene) {
                  if (!collision_on_potential_track) {
                    ...
                    if (
                      max_end - min_start <
                      placed_gene.display_range.width + this.display_range.width
                    ) {
            ...
            this.data[g].transcripts.map(
              function (d, t) {
                ...
                this.data[g].transcripts[t].exons.map(
                  function (d, e) {
                    this.data[g].transcripts[t].exons[e].parent =
                      this.data[g].transcripts[t]
                  }.bind(this)
                )
              }.bind(this)
            )
          ...
          .each(function (gene) {
            ...
            x = function (d) {
              if (d.display_range.text_anchor == 'middle') {
                ...
                return (
                  d.display_range.start + data_layer.layout.bounding_box_padding
                )
              } else if (d.display_range.text_anchor == 'end') {
                return (
                  d.display_range.end - data_layer.layout.bounding_box_padding
                )
              }
            }
            y = function (d) {
            ...
            var exons = d3
              ...
              .data(
                gene.transcripts[gene.parent.transcript_idx].exons,
                function (d) {
                  return d.exon_id
                }
              )
      ...
      this.positionTooltip = function (id) {
        ...
        // pad to either side if bumping up against the edge of the data layer
        var gene_center_x =
          (tooltip.data.display_range.start + tooltip.data.display_range.end) /
            2 -
        ...
        var left =
          ...
          offset_right
        var arrow_left =
          tooltip_box.width / 2 - arrow_width / 2 + offset_left - offset_right
    ...
    LocusZoom.DataLayers.add('intervals', function (layout) {
      ...
      this.assignTracks = function () {
        ...
        if (this.layout.track_split_field && this.layout.split_tracks) {
          this.data.map(
            function (d) {
              this.track_split_field_index[d[this.layout.track_split_field]] =
                null
            }.bind(this)
          )
          ...
          function (d, i) {
            ...
            this.data[i].display_range = {
              start: this.parent.x_scale(
                Math.max(d[this.layout.start_field], this.state.start)
              ),
              end: this.parent.x_scale(
                Math.min(d[this.layout.end_field], this.state.end)
              ),
            }
            this.data[i].display_range.width =
              this.data[i].display_range.end - this.data[i].display_range.start
              ...
              while (this.data[i].track == null) {
                ...
                this.interval_track_index[potential_track].map(
                  function (placed_interval) {
                    if (!collision_on_potential_track) {
                      var min_start = Math.min(
                        placed_interval.display_range.start,
                        this.display_range.start
                      )
                      var max_end = Math.max(
                        placed_interval.display_range.end,
                        this.display_range.end
                      )
                      if (
                        max_end - min_start <
                        placed_interval.display_range.width +
      ...
      this.render = function () {
          ...
          Object.keys(this.track_split_field_index).forEach(
            function (key) {
              ...
              this.group_hover_elements[key]
                ...
                .attr(
                  'height',
                  this.getTrackHeight() - this.layout.track_vertical_spacing
                )
                .attr('x', 0)
                .attr(
                  'y',
        ...
        selection
          ...
          .each(function (interval) {
            ...
            var rects = d3
              .select(this)
              .selectAll('rect.lz-data_layer-intervals.lz-interval_rect')
              .data([interval], function (d) {
            ...
            var clickareas = d3
              .select(this)
              .selectAll('rect.lz-data_layer-intervals.lz-clickarea')
              .data([interval], function (d) {
      ...
      this.positionTooltip = function (id) {
        ...
        if (this.layout.split_tracks) {
          ...
          interval_bbox = d3
            .select('#' + this.getElementId(tooltip.data) + '_bounding_box')
            .node()
            .getBBox()
        }
        var data_layer_height =
        ...
        var offset_left = Math.max(
          tooltip_box.width / 2 + interval_center_x - data_layer_width,
          0
        )
        var left =
          page_origin.x +
      ...
      this.toggleSplitTracks = function () {
        ...
        this.layout.group_hover_elements_on_field = this.layout.split_tracks
...
*/
    ...
    LocusZoom.Dashboard.Component.prototype.show = function () {
      ...
      if (!this.selector) {
        var group_position =
          ['start', 'middle', 'end'].indexOf(this.layout.group_position) != -1
    ...
    LocusZoom.Dashboard.Components = (function () {
      ...
      obj.get = function (name, layout, parent) {
        if (!name) {
          ...
          if (typeof layout != 'object') {
            throw (
              'invalid layout argument for dashboard component [' + name + ']'
            )
          } else {
      ...
      this.getClass = function () {
        var group_position =
          ['start', 'middle', 'end'].indexOf(
            ...
            ? ' lz-dashboard-button-group-' + this.parent.layout.group_position
            : ''
        return (
      ...
      this.menu = {
        ...
        position: function () {
          ...
          var base_max_width = Math.max(
            this.parent_svg.layout.width - 2 * padding - scrollbar_padding,
            scrollbar_padding
          )
          var container_max_width = base_max_width
          var content_max_width = base_max_width - 4 * padding
          var base_max_height = Math.max(
            this.parent_svg.layout.height - 10 * padding - menu_height_padding,
            menu_height_padding
          )
    ...
    LocusZoom.Dashboard.Components.add('title', function (layout) {
      ...
      this.show = function () {
        this.div_selector = this.parent.selector
          ...
          .attr(
            'class',
            'lz-dashboard-title lz-dashboard-' + this.layout.position
          )
    ...
    LocusZoom.Dashboard.Components.add('download', function (layout) {
      ...
      this.update = function () {
        ...
        this.button = new LocusZoom.Dashboard.Component.Button(this)
          ...
          .setTitle('Download image of the current plot as locuszoom.svg')
          .setOnMouseover(
            function () {
            ...
            function (response) {
              ...
              if (this.css_string.indexOf('/* ! LocusZoom HTML Styles */')) {
                this.css_string = this.css_string.substring(
                  0,
                  this.css_string.indexOf('/* ! LocusZoom HTML Styles */')
                )
              }
            }.bind(this)
          )
        ...
        return Q.fcall(
          function () {
            ...
            container.selectAll('g.tick text').each(function () {
              var dy =
                +d3.select(this).attr('dy').substring(-2).slice(0, -2) * 10
          ...
          .setOnclick(
            function () {
              if (
                ...
                d3.select(panel.parent.svg.node().parentNode).on(
                  'mouseover.' + panel.getBaseId() + '.dashboard',
                  null
                )
                d3.select(panel.parent.svg.node().parentNode).on(
                  'mouseout.' + panel.getBaseId() + '.dashboard',
                  null
                )
    ...
    LocusZoom.Dashboard.Components.add('shift_region', function (layout) {
      ...
      if (typeof layout.button_title != 'string') {
        layout.button_title =
          'Shift region by ' +
          (layout.step > 0 ? '+' : '-') +
          LocusZoom.positionIntToString(Math.abs(layout.step), null, true)
      }
      this.update = function () {
        if (this.button) {
          return this
      ...
      this.update = function () {
        if (this.button) {
          ...
          if (
            ...
            current_region_scale >= this.parent_plot.layout.max_region_scale
          ) {
            can_zoom = false
          }
          if (
            ...
            current_region_scale <= this.parent_plot.layout.min_region_scale
          ) {
            can_zoom = false
          }
    ...
    LocusZoom.Dashboard.Components.add('covariates_model', function (layout) {
        ...
        this.parent_plot.CovariatesModel = {
          ...
          add: function (element_reference) {
            ...
            if (
              ...
              element.html =
                typeof element_reference.toHTML == 'function'
            ...
            for (var i = 0; i < this.state.model.covariates.length; i++) {
              if (
                JSON.stringify(this.state.model.covariates[i]) ===
      ...
      this.update = function () {
          ...
          function () {
            ...
            if (!this.parent_plot.state.model.covariates.length) {
              ...
              selector
                ...
                .attr(
                  'class',
                  'lz-dashboard-button lz-dashboard-button-' + this.layout.color
                )
        ...
        this.button.preUpdate = function () {
          ...
          if (this.parent_plot.state.model.covariates.length) {
            var cov =
              this.parent_plot.state.model.covariates.length > 1
                ? 'covariates'
                : 'covariate'
            text +=
              ' (' +
            ...
            this.parent_panel.data_layer_ids_by_z_index
              ...
              .forEach(
                function (id, idx) {
                  ...
                  var name =
                    typeof data_layer.layout.name != 'string'
                      ? data_layer.id
                  ...
                  layout.statuses.forEach(
                    function (status_adj) {
                      ...
                      var status_verb =
                        LocusZoom.DataLayer.Statuses.verbs[status_idx]
                      var text, onclick, highlight
                      if (data_layer.global_statuses[status_adj]) {
                        text =
                          LocusZoom.DataLayer.Statuses.menu_antiverbs[
                            status_idx
                          ]
                  ...
                  var at_bottom =
                    idx ==
                    this.parent_panel.data_layer_ids_by_z_index.length - 1
            ...
            if (Array.isArray(this.parent.data_layers[id].layout.legend)) {
              this.parent.data_layers[id].layout.legend.forEach(
                function (element) {
                  ...
                  var label_size =
                    +element.label_size || +this.layout.label_size || 12
                  var label_x = 0
    ...
    LocusZoom.Legend.prototype.position = function () {
      ...
      if (!isNaN(+this.layout.pad_from_bottom)) {
        this.layout.origin.y =
          this.parent.layout.height - bcr.height - +this.layout.pad_from_bottom
      }
      if (!isNaN(+this.layout.pad_from_right)) {
        this.layout.origin.x =
          this.parent.layout.width - bcr.width - +this.layout.pad_from_right
      }
      this.selector.attr(
        'transform',
        'translate(' + this.layout.origin.x + ',' + this.layout.origin.y + ')'
      )
    }
...
*/
    ...
    LocusZoom.Data.Source.prototype.parseObjectsToObjects = function (
      ...
      fieldFound.forEach(function (v, i) {
        if (!v) {
          throw (
            'field ' + fields[i] + ' not found in response for ' + outnames[i]
          )
        }
      })
      return records
...
*/
    LocusZoom.Data.AssociationSource = LocusZoom.Data.Source.extend(function (
      init
    ) {
      this.parseInit(init)
    },
    ...
    LocusZoom.Data.AssociationSource.prototype.getURL = function (
      ...
      var analysis =
        state.analysis || chain.header.analysis || this.params.analysis || 3
      return (
        this.url +
...
*/
    ...
    LocusZoom.Data.LDSource.prototype.findMergeFields = function (chain) {
      ...
      if (chain && chain.body && chain.body.length > 0) {
        ...
        dataFields.id =
          dataFields.id || nameMatch(/\bvariant\b/) || nameMatch(/\bid\b/)
        dataFields.position =
          dataFields.position || nameMatch(/\bposition\b/i, /\bpos\b/i)
        dataFields.pvalue =
          dataFields.pvalue || nameMatch(/\bpvalue\b/i, /\blog_pvalue\b/i)
    ...
    LocusZoom.Plot = function (id, datasource, layout) {
      ...
      this.on = function (event, hook) {
        if (
          ...
          throw (
            'Unable to register event hook, invalid event: ' + event.toString()
          )
        }
      ...
      this.emit = function (event, context) {
        if (
          ...
          throw (
            'LocusZoom attempted to throw an invalid event: ' + event.toString()
          )
        }
...
*/
    LocusZoom.Plot.prototype.positionPanels = function () {
      ...
      this.panel_ids_by_y_index.forEach(
        function (panel_id) {
          this.panels[panel_id].layout.proportional_origin.y =
            this.panels[panel_id].layout.origin.y / calculated_plot_height
        }.bind(this)
      )
                ...
                panel_resize_drag.on(
                  ...
                  function () {
                    ...
                    this_panel.setDimensions(
                      ...
                      this_panel.layout.height + d3.event.dy
                    )
                    var panel_height_change =
                      this_panel.layout.height - original_panel_height
                    var new_calculated_plot_height =
                      this.parent.layout.height + panel_height_change
                    ...
                    this.parent.panel_ids_by_y_index.forEach(
                      function (loop_panel_id, loop_panel_idx) {
                        var loop_panel =
                          this.parent.panels[
                            this.parent.panel_ids_by_y_index[loop_panel_idx]
                          ]
                        loop_panel.layout.proportional_height =
                          loop_panel.layout.height / new_calculated_plot_height
                        if (loop_panel_idx > panel_idx) {
                          loop_panel.setOrigin(
                            loop_panel.layout.origin.x,
                            loop_panel.layout.origin.y + panel_height_change
                          )
        ...
        position: function () {
          ...
          this.selectors.forEach(
            function (selector, panel_idx) {
              ...
              var top =
                panel_page_origin.y +
                this.parent.panels[this.parent.panel_ids_by_y_index[panel_idx]]
                  .layout.height -
                12
                  ...
                  function (data_layer_id) {
                    ...
                    for (var property in this.state[state_id]) {
                      ...
                      if (Array.isArray(this.state[state_id][property])) {
                        this.state[state_id][property].forEach(
                          function (element_id) {
                            try {
                              this.setElementStatus(
                                property,
                                this.getElementById(element_id),
                                true
                              )
                            } catch (e) {
                              console.error(
    ...
    LocusZoom.Plot.prototype.stopDrag = function () {
      ...
      var overrideAxisLayout = function (axis, axis_number, extent) {
        panel.data_layer_ids_by_z_index.forEach(function (id) {
          if (
            panel.data_layers[id].layout[axis + '_axis'].axis == axis_number
          ) {
...
*/
    ...
    LocusZoom.Panel.prototype.setDimensions = function (width, height) {
      if (typeof width != 'undefined' && typeof height != 'undefined') {
        ...
        if (this.layout.proportional_width != null) {
          this.layout.width = Math.max(
            this.layout.proportional_width * this.parent.layout.width,
            this.layout.min_width
          )
        }
        if (this.layout.proportional_height != null) {
          this.layout.height = Math.max(
            this.layout.proportional_height * this.parent.layout.height,
            this.layout.min_height
          )
        }
      }
    ...
    LocusZoom.Panel.prototype.getLinkedPanelIds = function (axis) {
      ...
      this.parent.panel_ids_by_y_index.forEach(
        function (panel_id) {
          if (
            panel_id != this.id &&
            this.parent.panels[panel_id].layout.interaction[axis + '_linked']
          ) {
    ...
    LocusZoom.Panel.prototype.addDataLayer = function (layout) {
      ...
      if (
        ...
        if (data_layer.layout.z_index < 0) {
          data_layer.layout.z_index = Math.max(
            this.data_layer_ids_by_z_index.length + data_layer.layout.z_index,
            0
          )
        }
      ...
      if (layout_idx == null) {
        layout_idx =
          this.layout.data_layers.push(this.data_layers[data_layer.id].layout) -
          1
      }
      ...
      for (var id in this.data_layers) {
        ...
        if (data_layer.layout.x_axis && !data_layer.layout.x_axis.decoupled) {
          this.x_extent = d3.extent(
            (this.x_extent || []).concat(data_layer.getAxisExtent('x'))
          )
        }
    ...
    LocusZoom.Panel.prototype.render = function () {
      ...
      if (
        ...
        (this.parent.interaction.panel_id == this.id ||
          this.parent.interaction.linked_panel_ids.indexOf(this.id) != -1)
      ) {
        var anchor,
          scalar = null
        if (
          ...
          var new_x_extent_start = Math.max(
            Math.floor(
              this.x_scale.invert(ranges.x_shifted[0]) -
                (new_extent_size - current_scaled_extent_size) * offset_ratio
            ),
            1
          )
          ranges.x_shifted = [
          ...
          switch (this.parent.interaction.dragging.method) {
            ...
            case 'x_tick':
              if (d3.event && d3.event.shiftKey) {
                ...
                scalar = constrain(
                  anchor /
                    (anchor + this.parent.interaction.dragging.dragged_x),
                  3
                )
            ...
            case 'y2_tick':
              var y_shifted =
                'y' + this.parent.interaction.dragging.method[1] + '_shifted'
              if (d3.event && d3.event.shiftKey) {
                ranges[y_shifted][0] =
                  this.layout.cliparea.height +
                  this.parent.interaction.dragging.dragged_y
                ranges[y_shifted][1] =
                  0 + this.parent.interaction.dragging.dragged_y
              } else {
                anchor =
                  this.layout.cliparea.height -
                ...
                scalar = constrain(
                  anchor /
                    (anchor - this.parent.interaction.dragging.dragged_y),
                  3
                )
      ...
      if (this.layout.interaction.scroll_to_zoom) {
        var zoom_handler = function () {
          ...
          var delta = Math.max(
            ...
            Math.min(
              1,
              d3.event.wheelDelta || -d3.event.detail || -d3.event.deltaY
            )
          )
          if (delta == 0) {
            return
    ...
    LocusZoom.DataLayer.Statuses.verbs.forEach(function (verb, idx) {
      ...
      LocusZoom.Panel.prototype[antiverb + 'ElementsByFilters'] = function (
        filters,
        exclusive
      ) {
        if (typeof exclusive == 'undefined') {
