...
def outputschedule(rownum, totalnum,current_analysis_order, total_numof_analyses, rank_dir):
    ...
    logging.info(f"process ecaviar schedual: {calculated_schedule}")
...
class ECaviarDataProcessor:
    ...
    def __init__(self):
        logging.info('init ECaviarDataProcessor')
    ...
    def prepare(self, working_dir, gwas_cluster_dir, gwas_cluster_summary, 
                ...
                whether_schedual=False, min_matching_number = 0,
            ...
            with ThreadPoolExecutor(max_workers=parallel_worker_num) as executor:
                ...
                for gwas_cluster_file in os.listdir(gwas_cluster_dir):
                    ...
                    if Path(f'{qtl_group_dir}/{chromosome}').exists():
                        for qtl_file in os.listdir(f'{qtl_group_dir}/{chromosome}'):
                            # gene_id = '.'.join(qtl_file.split('.')[:2])
                            gene_id = utils.get_eqtl_gene_name(qtl_file)
                            # eqtl一个基因文件中显著位点的positions
                            qtl_significant_positions = self.__get_qtl_significant_snp_positions(
                                qtl_candidate_df, chromosome, qtl_file)
                            # 如果当前基因文件中至少有一个显著位点存在于当前gwas的显著位点里，才进行gwas和eqtl的位点mapping工作
                            if variant_id not in set(gwas_cluster_snps_dict.keys()):
                                logging.info(f"ecaviar range_lead_snp {variant_id} not in set(gwas_cluster_snps_dict.keys())")
                                continue
                            if len(set(gwas_cluster_snps_dict[variant_id]) & set(qtl_significant_positions)) == 0:
                                continue
                            # if not gene_id.upper().startswith(const.gene_id_prefix):
                            #     continue
                            gwas_cluster_full_path = f'{gwas_cluster_dir}/{gwas_cluster_file}'
                            gene_file_full_path = f'{qtl_group_dir}/{chromosome}/{qtl_file}'
                            input_vcf = os.path.join(ref_vcf_dir, population.upper(), f'chr{chromosome}.vcf.gz')
                            print(f"ecaviar input_vcf: {input_vcf}")
                            futures.append(executor.submit(self.process_gene, working_dir, gwas_cluster_full_path,
                                                        gene_file_full_path, input_vcf, gwas_col_dict, qtl_col_dict,
                                                        gwas_sample_size, qtl_sample_size, var_id_col_name,
                                                        output_base_dir, gene_id, variant_id, chromosome, min_matching_number, gwas_threshold, qtl_threshold))
    ...
    def process_gene(self, working_dir, gwas_cluster_file_path, gene_file_path, input_vcf,
        ...
        gwas_cluster_df = pd.read_csv(gwas_cluster_file_path, sep=const.column_spliter,
                                      usecols=[gwas_col_dict['chrom'], gwas_col_dict['position'],
                                               ...
                                               gwas_col_dict['beta'], gwas_col_dict['se'], gwas_col_dict['pvalue'], var_id_col_name],
                                      dtype={
        ...
        qtl_grouped_df = pd.read_csv(gene_file_path, sep=const.column_spliter,
        ...
        # logging.info(f"ecaviar qtl_grouped_df.shape: {qtl_grouped_df.shape}")
        qtl_grouped_df.sort_values(by=qtl_col_dict['position'], inplace=True)
        qtl_grouped_df.reset_index(drop=True, inplace=True)
        qtl_zscore_file = f'{candidate_dir}/qtl_{variant_id}_{gene_id}.z'
        ...
        # logging.info(f"ecaviar qtl_grouped_df.columns: {qtl_grouped_df.columns}")

        if len(qtl_grouped_df[qtl_grouped_df[qtl_col_dict['pvalue']] < qtl_threshold]) <= 0:
        ...
        gwas_cluster_df = pd.merge(left=gwas_cluster_df,
                        ...
                        how='left')
        if len(gwas_cluster_df[gwas_cluster_df[gwas_col_dict['pvalue']] < gwas_threshold]) <= 0:
        ...
        try: 
            with open(finemap_in_file, mode='w') as in_file:
                ...
                in_file.write(f'{gwas_zscore_file};{output_ld_file}.ld;'
                            ...
                            f'{candidate_dir}/gwas_{variant_id}_{gene_id}.cred;'
                            f'{candidate_dir}/gwas_log.log;{gwas_sample_size}\n')
                in_file.write(f'{qtl_zscore_file};{output_ld_file}.ld;'
                            f'{candidate_dir}/qtl_{variant_id}_{gene_id}.snp;'
                            f'{candidate_dir}/qtl_{variant_id}_{gene_id}.config;'
                            f'{candidate_dir}/qtl_{variant_id}_{gene_id}.cred;'
                            f'{candidate_dir}/qtl_log.log;{qtl_sample_size}')
        except OSError as e:
            # 捕获文件无法打开的错误
            logging.info(f"Error: File '{finemap_in_file}' cannot be opened! {e}")
    ...
    def __get_qtl_significant_snp_positions(self, qtl_df, chrom, gene_file_name):
        results = qtl_df[(qtl_df['pheno_file'] == gene_file_name) & (qtl_df['chrom'].astype(str) == str(chrom))]
        if results.empty:
            return []
        else:
...
if __name__ == '__main__':
    ...
    gwas_data_processor.prepare(working_dir=_working_dir,
                                ...
                                qtl_group_dir='/Volumes/HD/biodata/colocalization-tools/preprocessed/eqtl/Artery_Aorta/grouped',
                                qtl_report='/Volumes/HD/biodata/colocalization-tools/preprocessed/eqtl/Artery_Aorta/filtered_gene.tsv',
