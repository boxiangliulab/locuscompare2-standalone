...
class SmrQTLProcessor:
        ...
        if parallel:
            with ThreadPoolExecutor(max_workers=parallel_worker_num) as executor:
                ...
                for ix, row in eqtl_summary_df.iterrows():
                    ...
                    gene_id = utils.get_eqtl_gene_name(qtl_gene_file)
                    eqtl_positions = ast.literal_eval(row.loc['positions'])
                    if len(eqtl_positions) < min_matching_number:
                        continue
                    if not pval_filtered_gwas_df[\
                        pval_filtered_gwas_df[gwas_col_dict['chrom']] == chrom].\
                        loc[:, gwas_col_dict['position']].isin(eqtl_positions).any():
                        continue
                    input_vcf = os.path.join(ref_vcf_dir, population.upper(), f'chr{chrom}.vcf.gz')
                    print(f"smr input_vcf: {input_vcf}")
                    futures.append(executor.submit(self.gen_plink_binary_ld_ref, working_dir, qtl_gene_file,
                                                   var_id_col_name, qtl_col_dict, chrom, gene_id, input_vcf,
                                                   min_matching_number, gwas_threshold, qtl_threshold))
            ...
            for ix, row in eqtl_summary_df.iterrows():
                ...
                if len(eqtl_positions) < min_matching_number:
                    continue
                if not pval_filtered_gwas_df[pval_filtered_gwas_df\
                    [gwas_col_dict['chrom']] == chrom].loc\
                ...
                self.gen_plink_binary_ld_ref(working_dir, qtl_gene_file, var_id_col_name, qtl_col_dict,
                                             chrom, gene_id, input_vcf, min_matching_number, gwas_threshold, qtl_threshold)
    ...
    def gen_plink_binary_ld_ref(self, working_dir, qtl_gene_file, var_id_col_name, qtl_col_dict,
        ...
        # candidate_qtl_trait_df = utils.filter_data_frame_by_p_value(qtl_trait_df, eqtl_p_thresh,
        ...
        #         inplace=False)
        if len(qtl_trait_df[qtl_trait_df[qtl_col_dict['pvalue']] < qtl_threshold]) <= 0:
        ...
        if len(vcf_matching_df) < min_matching_number:
            logging.info(f"len(vcf_matching_df) < min_matching_number {min_matching_number}")
            return
        ...
        if len(candidate_qtl_trait_df) < min_matching_number:
            logging.info(f"len(candidate_qtl_trait_df) < min_matching_number {min_matching_number}")
            return
        ...
        logging.info(f'plink --silent --vcf {output_vcf_full_path} --make-bed --snps-only --out {output_ld_ref_path}')
...
if __name__ == '__main__':
    ...
    smr_eqtl_processor = SmrQTLProcessor()
