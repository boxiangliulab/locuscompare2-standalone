        ...
        for chrom_group_file in os.listdir(gwas_chrom_group_dir):
            ...
            for _, row in qtl_summary_df.iterrows():
                ...
                if not pval_filtered_gwas_df[pval_filtered_gwas_df\
                                            [gwas_col_dict['chrom']] == chrom].\
                                            loc[:, gwas_col_dict['position']].\
                                            isin(qtl_positions).any():
                    logging.debug(
                        f'No intersection between gwas risk SNPs and qtl risk SNPs for gene {gene_id}: {datetime.now()}')
                    continue
                ...
                qtl_input_path = os.path.join(input_dir, f'qtl_chr{chrom}_{gene_id}.tsv')
                ...
                candidate_gwas_df.reset_index(drop=True, inplace=True)
                candidate_qtl_trait_df.reset_index(drop=True, inplace=True)
                qtl_flist_file = os.path.join(input_dir, f'{gene_id}_flist.txt')
                qtl_besd_file = os.path.join(input_dir, f'{gene_id}')
                ...
                logging.info(f'qtl_flist_file: {qtl_flist_file}')
                with open(qtl_flist_file, mode='w') as flist_ptr:
                    ...
                    flist_ptr.write(
                        f'{chrom}\t{gene_id}\t0\t{probe_bp}\t{gene_name}\t{gene_strand}\t{qtl_input_path}\n')
                ...
                logging.info(f"gene_out_result: {gene_out_result}")
                ...
                logging.info(f"{smr_cmd} {cmd_params}")
