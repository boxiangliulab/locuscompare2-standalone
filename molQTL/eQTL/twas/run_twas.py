...
class TWAS:
    ...
    def run(self,
        ...
        if parallel:
            logging.info(f"fusion parallel")
            with ThreadPoolExecutor(max_workers=parallel_worker_num) as executor:
                ...
                for chrom_group_file in os.listdir(gwas_chrom_group_dir):
                    ...
                    futures.append(
                        executor.submit(self.process_chrom, chrom, chrom_group_file_path, chrom_group_file, ld_ref_prefix,
            ...
            logging.info(f"fusion not parallel")
            for chrom_group_file in os.listdir(gwas_chrom_group_dir):
                chrom = self.__ensure_chrom_ld_ref(chrom_group_file, ref_vcf_dir, population)
                if chrom is None:
                    continue
                chrom_group_file_path = os.path.join(gwas_chrom_group_dir, chrom_group_file)
                self.process_chrom(chrom, chrom_group_file_path, chrom_group_file, ld_ref_prefix, gwas_col_dict, input_dir,
    ...
    def process_chrom(self, chrom, gwas_file, file_name, ld_ref_prefix, gwas_col_dict, input_dir, twas_path,
        ...
        gwas_chrom_input = os.path.join(input_dir, file_name)
        ...
        chrom_out_file = os.path.join(input_dir, f"{file_base_name}_result.tsv")
        ...
        logging.info(f'Rscript --no-save --no-restore {twas_path} '
