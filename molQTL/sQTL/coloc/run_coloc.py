...
class Coloc:
    ...
    def run(self,
        ...
        # print(f"qtl_summary_df: {qtl_summary_df}")
        ...
        if parallel:
            with ThreadPoolExecutor(max_workers=parallel_worker_num) as executor:
                ...
                for ix, row in qtl_summary_df.iterrows():
                    ...
                    phenotype_id = utils.get_file_name(qtl_gene_file)
                    ...
                    for gwas_range_file in gwas_range_files[chrom]:
                        ...
                        # logging.info(f"range_lead_snp: {range_lead_snp}")
                        if len(set(gwas_cluster_snps_dict[range_lead_snp]) & set(qtl_significant_positions)) < min_matching_number:
                            continue
                        print(f"pass")
                        futures.append(executor.submit(self.process_pheno, 
                                                       self.__get_output_dir(working_dir),
                                                       gwas_range_file, 
                                                       gwas_type_dict, 
                                                       gwas_col_dict, row,
                                                       qtl_gene_file, 
            ...
            for ix, row in qtl_summary_df.iterrows():
                ...
                for gwas_range_file in gwas_range_files[chrom]:
                    ...
                    if len(set(gwas_cluster_snps_dict[range_lead_snp]) & set(qtl_significant_positions)) < min_matching_number:
                        continue
                    print(f"pass")
                    self.process_pheno(self.__get_output_dir(working_dir), 
                                       gwas_range_file, gwas_type_dict,
                                       gwas_col_dict, row, qtl_gene_file, 
                                       qtl_type_dict, var_id_col_name, 
                                       coloc_input_dir, phenotype_id, gene_id, 
                                       qtl_col_dict, gwas_sample_size,
                                       qtl_sample_size, gwas_type, qtl_type, 
                                       _p1, _p2, _p12, min_matching_number)
    ...
    def process_pheno(self, output_dir, gwas_range_file, gwas_type_dict, 
                      gwas_col_dict, row, qtl_gene_file, qtl_type_dict, 
                      var_id_col_name, coloc_input_dir, phenotype_id, gene_id, 
                      qtl_col_dict, gwas_sample_size, qtl_sample_size, 
                      gwas_type, qtl_type, p1, p2, p12, min_matching_number):
        ...
        if len(qtl_trait_df) <= min_matching_number:
            logging.info(f'no qtl_trait_df') # 有问题
            return
        ...
        if len(candidate_gwas_df) <= min_matching_number:
            return
        coloc_gwas_input_path = os.path.join(coloc_input_dir, f'gwas_{gene_id}_{phenotype_id}_{range_lead_snp}.tsv.gz')
        coloc_qtl_input_path = os.path.join(coloc_input_dir, f'qtl_{gene_id}_{phenotype_id}_{range_lead_snp}.tsv.gz')
        output_file = os.path.join(output_dir, f'{gene_id}_{phenotype_id}_{range_lead_snp}.tsv')
        ...
        print(f'Rscript --no-save --no-restore {rscript_path} '
