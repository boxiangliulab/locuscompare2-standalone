...
#original recognized type: 'protein_coding', 'pseudogene', 'lincRNA'
#all type: 'transcribed_unprocessed_pseudogene', 'unprocessed_pseudogene', 'miRNA', 'lincRNA', 'protein_coding', 'processed_pseudogene', 'antisense', 'processed_transcript', 'snRNA', 'transcribed_processed_pseudogene', 'sense_intronic', 'misc_RNA', 'TEC', 'transcribed_unitary_pseudogene', 'snoRNA', 'scaRNA', 'rRNA', 'unitary_pseudogene', '3prime_overlapping_ncRNA', 'polymorphic_pseudogene', 'bidirectional_promoter_lncRNA', 'sense_overlapping', 'pseudogene', 'IG_V_pseudogene', 'scRNA', 'IG_V_gene', 'IG_C_gene', 'IG_J_gene', 'sRNA', 'ribozyme', 'vaultRNA', 'non_coding', 'TR_C_gene', 'TR_J_gene', 'TR_V_gene', 'TR_V_pseudogene', 'TR_D_gene', 'IG_C_pseudogene', 'macro_lncRNA', 'TR_J_pseudogene', 'IG_J_pseudogene', 'IG_D_gene', 'IG_pseudogene', 'Mt_tRNA', 'Mt_rRNA'
get_gene_annotation <- function(gene_annot_file_name, chrom, is_simu_data=FALSE, gene_types=NA){
  if(is.na(gene_types)){
    if(is_simu_data){
      gene_types = c('transcribed_unprocessed_pseudogene', 'unprocessed_pseudogene', 'miRNA', 'lincRNA', 'protein_coding', 'processed_pseudogene', 'antisense', 'processed_transcript', 'snRNA', 'transcribed_processed_pseudogene', 'sense_intronic', 'misc_RNA', 'TEC', 'transcribed_unitary_pseudogene', 'snoRNA', 'scaRNA', 'rRNA', 'unitary_pseudogene', '3prime_overlapping_ncRNA', 'polymorphic_pseudogene', 'bidirectional_promoter_lncRNA', 'sense_overlapping', 'pseudogene', 'IG_V_pseudogene', 'scRNA', 'IG_V_gene', 'IG_C_gene', 'IG_J_gene', 'sRNA', 'ribozyme', 'vaultRNA', 'non_coding', 'TR_C_gene', 'TR_J_gene', 'TR_V_gene', 'TR_V_pseudogene', 'TR_D_gene', 'IG_C_pseudogene', 'macro_lncRNA', 'TR_J_pseudogene', 'IG_J_pseudogene', 'IG_D_gene', 'IG_pseudogene', 'Mt_tRNA', 'Mt_rRNA')
    } else {
...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
                 ...
                 seed=NA, cis_window=1e6, alpha=0.5, null_testing=FALSE, summary_dir=NA, weights_dir=NA, covariances_dir=NA, is_simu_data=NA) {
  gene_annot <- get_gene_annotation(gene_annot_file, chrom, is_simu_data)
  ...
  if(is.na(covariances_dir)){
    covariances_dir = "./covariances"
  }
  if(!dir.exists(covariances_dir)){
    dir.create(covariances_dir, showWarnings = FALSE)
  }

  # Prepare output data----
  model_summary_file <- file.path(summary_dir, prefix %&% '_chr' %&% chrom %&% '_model_summaries.txt')
  ...
  weights_file <- file.path(weights_dir, prefix %&% '_chr' %&% chrom %&% '_weights.txt')
  weights_col <- c('gene_id', 'rsid', 'varID', 'ref', 'alt', 'beta')
  write(weights_col, file = weights_file, ncol = 6, sep = '\t')

  tiss_chr_summ_f <- file.path(summary_dir, prefix %&% '_chr' %&% chrom %&% '_summary.txt')
  ...
  covariance_file <- file.path(covariances_dir, prefix %&% '_chr' %&% chrom %&% '_covariances.txt')
...
snp_annot_file = argv[1]
gene_annot_file = argv[2]
genotype_file = argv[3]
expression_file = argv[4]
covariates_file = argv[5]
chrom = argv[6]
summary_dir = argv[7]
weights_dir = argv[8]
covariances_dir = argv[9]
is_simulation_data = as.logical(argv[10])
if (is.na(is_simulation_data)) {
...
main(snp_annot_file, gene_annot_file, genotype_file, expression_file, covariates_file, as.numeric(chrom), prefix, null_testing=FALSE, summary_dir=summary_dir, weights_dir=weights_dir, covariances_dir=covariances_dir, is_simu_data=is_simulation_data)
